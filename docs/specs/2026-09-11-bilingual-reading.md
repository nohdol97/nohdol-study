...
5. Every glossary entry explains why it matters and when to use it in both languages, separately from its definition. Chapter purposes remain in Markdown definition tables or lists. Core entries, including CDC and dbt, also include a small example, distinctions from adjacent tools, and source/context links. Missing English or Korean purpose rejects publication. Terms are not inserted into code or links.
...
The 22 site tests cover all 94 translations, shared fenced code/results/diagrams, source-comment preservation, internal destinations, structural drift, stale source bindings, private and untracked paths, escaping symlinks, and reading preferences. Existing executable course fixtures also pass. The glossary extracts 403 chapter definitions with 375 distinct labels and adds 14 core technology explainers with examples and role comparisons. All entries have bilingual purpose copy. Tests check extraction separately from meaning, Markdown retention, complete coverage, and rejection of one-sided or missing purpose. Chapter definitions retain their local context rather than merging ambiguous meanings.
...
The purpose extension opens all 1,151 term dialogs offered across the 94 article routes and checks that each includes both language paragraphs under its purpose heading. The count includes definitions reused in related chapters. A source comparison confirms that the 132 edited English/Korean Markdown files retain their original definitions, examples, source records, and structure while adding purpose columns or list suffixes.
