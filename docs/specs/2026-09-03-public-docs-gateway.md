...
- 게이트웨이 초기 구현은 Kubernetes 하나로 시작해 공식 문서를 이해 순서로 재구성한 전체 로드맵과 10개 장을 제공한다. 이후 주제 확장은 [Infra Specialist 공개 학습 경로](2026-09-03-infra-specialist-public-learning-path.md)가 정의한다.
...
첫 화면은 사이트 목적, 전체 문서 검색과 카탈로그에 등록된 주제 카드를 제공한다. 새 주제는 독립된 콘텐츠 디렉터리와 목차를 가진 카드로 추가할 수 있다. 초기 카탈로그의 Kubernetes 카드를 선택하면 다음 11개 문서가 이 순서로 나타나고 언제든 전체 주제 게이트웨이로 돌아올 수 있다.
