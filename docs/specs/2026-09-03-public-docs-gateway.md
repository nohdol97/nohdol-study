...
공개 본문은 `docs-site/content/<topic>/` 아래에서 주제별로 분리한다. 현재 Kubernetes 본문은 `docs-site/content/kubernetes/`에 둔다. 각 문서는 선수·후속 장의 관계를 밝힌다. 링크를 상세화할 때는 한 문장 모델, 관계 및 시퀀스 다이어그램, 최소 실행 예시, 상세 해설, 실패 예시와 복구, 운영 판단, 복습 질문을 기본 구조로 사용한다.
