...
You already have a dataset, quality checks, and a recovery procedure. What changes if you run them on Databricks or Snowflake?

Implement the same requirements on each platform. Compare the results, recovery behavior, permissions, and cost using the same workload.

## Terms introduced in this chapter

| Term | Meaning | Why it matters / when to use it |
|---|---|---|
| Managed compute | Compute capacity for which a provider handles part of the setup and operation. | Delegate part of infrastructure operation while retaining responsibility for workload correctness and cost. **Concrete situation (illustrative):** A team wants to run data jobs without maintaining worker hosts. → Select managed compute with explicit scaling, runtime, and cost constraints. → Test workload behavior and remaining operational responsibilities. |
| Unity Catalog | Databricks' layer for finding and governing supported data and other assets. | Coordinate asset discovery and access governance across supported Databricks workloads. **Concrete situation (illustrative):** Databricks teams need consistent permissions and discovery for shared data. → Register and govern supported assets through Unity Catalog. → Test access with actual user and service identities at the intended scope. |
| Lakeflow | Databricks' set of capabilities for ingesting data, building pipelines, and coordinating jobs. | Coordinate ingestion, transformations, and jobs within Databricks' data-engineering tooling. **Concrete situation (illustrative):** A Databricks pipeline needs ingestion, transformation, and scheduled operation. → Map each requirement to the appropriate Lakeflow capability. → Verify dependencies, retry behavior, and data-quality outcomes for the selected components. |
| Dynamic table | A Snowflake table defined by a query whose results are kept updated through refreshes. | Maintain a query-defined analytical result through managed refresh behavior with explicit freshness requirements. **Concrete situation (illustrative):** A Snowflake team maintains derived tables with repetitive refresh logic. → Define a supported dynamic table and its target lag. → Observe actual freshness, refresh behavior, and compute cost under representative updates. |
| DMF | A Snowflake Data Metric Function measures a property of data, such as how many values are missing. | Measure data properties repeatedly so quality expectations can be evaluated on recorded results. **Concrete situation (illustrative):** A Snowflake table should be monitored for nulls and other quality conditions. → Associate suitable data metric functions and configure supported monitoring. → Verify returned measurements and separately defined expectations or alerts. |
| Warehouse | In Snowflake, a virtual warehouse supplies compute for supported workloads; it is not the stored data itself. | Assign and size Snowflake compute for a workload while managing concurrency and spending. **Concrete situation (illustrative):** An expensive transformation competes with interactive Snowflake queries. → Evaluate separate virtual warehouses and suitable sizing. → Compare workload latency and credit use under the intended concurrency. |
