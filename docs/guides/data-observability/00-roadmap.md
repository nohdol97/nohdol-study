...
Imagine that yesterday's sales report is wrong. You need to find where its data came from, which step failed, and how to correct the result.

This course builds one system around those questions. Start with orders and reports, then add distributed processing, monitoring, access control, and an AI assistant.
