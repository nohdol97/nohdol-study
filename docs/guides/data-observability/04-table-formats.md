...
A job should write twenty Parquet files, but it stops after twelve. A reader looking at the directory cannot tell whether those twelve files are the final result.

A table format records which files belong to an accepted table version. Its metadata and commit rules let readers distinguish a complete result from an unfinished write.

## Terms introduced in this chapter

| Term | Meaning | Why it matters / when to use it |
|---|---|---|
| Snapshot or version | A committed table state with an identifier, so a reader can request that particular state. | Reproduce a query or recovery decision against an identified committed table state. **Concrete situation (illustrative):** Yesterday's report must be reproduced after today's correction. → Read the retained table snapshot or version used by the original run. → Compare its identifier and output with the report record. |
| Manifest | An Iceberg metadata file that lists data files or delete files and information about them. | Plan Iceberg reads using recorded file metadata instead of discovering table membership from directory contents. **Concrete situation (illustrative):** An Iceberg query must determine which files belong to a snapshot. → Follow its manifest metadata and file statistics. → Confirm planning selects files for that snapshot and filter. |
| Catalog | A way for engines to find tables and their metadata. Its coordination duties depend on the implementation. | Locate the authoritative table metadata and coordinate updates under the catalog's commit contract. **Concrete situation (illustrative):** Two engines need to refer to the same lakehouse table. → Configure the intended catalog and namespace consistently. → Check that both resolve the same table and current metadata. |
| Optimistic concurrency | Preparing a change first, then checking for conflicting changes before committing or retrying it. | Allow concurrent preparation while rejecting or retrying changes that conflict at commit. **Concrete situation (illustrative):** Two writers update overlapping table state at the same time. → Detect commit conflicts and apply the engine's retry or failure policy. → Confirm neither writer silently overwrites the other's accepted changes. |
| Compaction | Rewriting many small or fragmented files into a more useful layout while preserving the intended rows. | Reduce excessive small-file overhead while preserving the intended table contents. **Concrete situation (illustrative):** Frequent ingestion leaves thousands of tiny data files. → Compact eligible files into a more suitable layout. → Compare scan overhead and verify row-level results remain equivalent. |
| Retention | How long old versions and the files needed to read them remain available. | Keep enough history for replay, investigation, and recovery without assuming storage can be unlimited. **Concrete situation (illustrative):** Storage cleanup is planned while analysts still need last month's version. → Set retention around recovery, readers, and policy requirements. → Test that required versions remain readable before removing eligible data. |
