...
A sales report needs only the order date and amount. Reading every customer name and address wastes work.

Parquet stores data by column so a reader can select the fields it needs. When the query and file metadata support it, the reader can also skip parts of the file.

## Terms introduced in this chapter

| Term | Meaning | Why it matters / when to use it |
|---|---|---|
| Row group | A batch of rows in a Parquet file. Within it, each column's data is stored in its own chunk. | Balance scan parallelism and statistics-based skipping against file-layout overhead. **Concrete situation (illustrative):** A Parquet reader needs only a narrow date range from a large file. → Inspect row-group statistics and layout. → Confirm irrelevant groups can be skipped for this query. |
| Column chunk | The data for one column inside a Parquet row group. | Locate and read the required column's bytes within a Parquet row group. **Concrete situation (illustrative):** A report reads two columns from a wide Parquet table. → Locate their column chunks within each selected row group. → Check that unrelated column data is not unnecessarily read. |
| Page | A smaller encoded piece of data inside a Parquet column chunk. | Understand the finer encoding and read units that affect compression and supported skipping. **Concrete situation (illustrative):** A Parquet scan still performs substantial decoding after selecting columns. → Examine page encoding and available page-level metadata. → Measure whether supported page skipping or encoding changes reduce work. |
| Column pruning | Reading only the columns a query needs, such as date and amount for a revenue report. | Avoid reading unrelated columns when a query needs only a small part of a wide table. **Concrete situation (illustrative):** A dashboard uses customer ID and revenue from a table with many text fields. → Select only the needed columns. → Compare bytes read with a query that selects every column. |
| Predicate pushdown | Passing a query's filter to the data reader so it can skip unnecessary work when supported. | Reduce data passed to later operators when the reader can evaluate or exploit the filter. **Concrete situation (illustrative):** A job loads a whole year's data before filtering one day. → Express the filter so the reader can apply supported pushdown. → Inspect the plan and scan metrics to confirm reduced reading. |
| Object key | The name that identifies an object within its storage container, such as a bucket. | Address a specific stored object and design naming that supports access and lifecycle policies. **Concrete situation (illustrative):** Two exports accidentally overwrite the same object-storage destination. → Define keys that distinguish dataset, partition, and intended version. → Verify retries follow the chosen overwrite or deduplication policy. |
