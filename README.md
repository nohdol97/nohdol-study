...
- **Korean, English, and paired reading**: All 94 articles include reviewed Korean text beside the English original. Collapse explanations to practice comprehension, search either language, and open terminology definitions with reasons and concrete situations. Every glossary entry connects a realistic problem to an application and a result to check, in both languages. CDC, dbt, and other core technologies also compare adjacent roles. Code, output fixtures, and diagrams appear once. Interface language is a separate saved preference. See the [bilingual reading specification](docs/specs/2026-09-11-bilingual-reading.md).
