...
- **한 줄 역할**: 지식 루트에서 어긋난 것을 보고한다 — 대상 없는 링크, 링크도 분류도 없는 노트, 노트 계약을 어긴 frontmatter, `raw/`에 없는 인용 출처, 예산 초과 세션 캐시.
- **언제 쓰나**: vault 점검, 깨진 링크·고아 노트 찾기, 노트 상태 점검, 주기적 정리.
...
- **판단 원칙**: 고아는 실제 관계가 있을 때만 연결한다. **목록을 비우려고 링크를 추가하지 않는다** — 장식용 링크는 그래프가 거짓을 말하게 해서 정직한 고아보다 나쁘다. `checked` 날짜 누락은 오늘 날짜를 쓰는 게 아니라 다시 확인해서 고친다.
