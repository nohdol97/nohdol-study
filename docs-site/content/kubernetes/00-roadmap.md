...
|---|---|---|
| container | A unit that isolates and executes application processes and files required for execution | Package dependencies with isolated processes so applications can run in repeatable environments. **Concrete situation (illustrative):** The API runs on a laptop but lacks a library on the server. → Package its dependencies in a container image. → Test startup in a clean environment. |
| image | Read-only execution material used when creating a container | Distribute the same reviewed application contents to development, testing, and production. **Concrete situation (illustrative):** Two servers run different application builds under the same tag. → Deploy a reviewed image digest. → Compare the running image identities. |
| cluster | A set of control planes and multiple servers managed together by Kubernetes | Coordinate placement and recovery across servers instead of managing each application host separately. **Concrete situation (illustrative):** An application must survive one server failing. → Spread replicas across eligible cluster Nodes. → Stop one test Node and check requests recover. |
| Node | The server where the container actually runs | Locate the compute capacity and host failures that determine whether workloads can run. **Concrete situation (illustrative):** Several Pods fail together although their images differ. → Inspect their shared Node's health and resources. → Check whether the failures follow that host. |
| Pod | A bundle of containers that Kubernetes deploys and manages together on one node. | Give tightly coupled containers a shared scheduling and lifecycle boundary. **Concrete situation (illustrative):** An API and its helper must share local files and lifecycle. → Place the coupled containers in one Pod. → Check shared-volume access and restart behavior. |
| Namespace | A logical boundary that divides the name, inquiry, authority, and policy scope of related resources within a cluster. | Organize teams' resources and scope names, permissions, and quotas within one cluster. **Concrete situation (illustrative):** Two teams both deploy an application named api. → Give each team a Namespace and scoped permissions. → Verify name resolution and access separately. |
| kubectl | Command-line tool that allows users to send inquiry/change requests to the Kubernetes API | Inspect cluster objects and submit declarative changes while diagnosing workload problems. **Concrete situation (illustrative):** A Pod remains Pending after deployment. → Inspect its description and events with kubectl. → Identify the recorded scheduling reason before changing resources. |
| desired state | A user-declared goal, such as “I need to keep three Pods ready” | Let controllers compare the target with reality and repair deviations repeatedly. **Concrete situation (illustrative):** A Deployment requests three replicas but only two are ready. → Compare its desired and observed state. → Watch whether reconciliation restores three healthy replicas. |
