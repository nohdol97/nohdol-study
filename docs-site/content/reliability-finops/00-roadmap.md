...
|---|---|---|
| availability | The extent to which users were actually able to use the functionality they needed | Measure whether users can complete required actions, not merely whether servers are running. **Concrete situation (illustrative):** Servers respond to health checks while users cannot place orders. → Measure successful order attempts. → Compare user availability with infrastructure health. |
| failure | A state in which part of a system fails to perform its expected function. | Identify a broken function precisely so recovery targets the user-visible problem. **Concrete situation (illustrative):** Only receipt delivery fails while payment still succeeds. → Identify the failed function and its dependency. → Verify recovery for receipts without disrupting payments. |
| Redundancy | A design that places the same role in multiple places so that operation continues even if one fails | Keep an alternate serving path when one component fails, while checking shared dependencies. **Concrete situation (illustrative):** Two replicas share the same failing storage dependency. → Review redundancy across actual failure boundaries. → Test whether the alternate path survives that dependency loss. |
| RTO | Target time allowed for service to be available again after a failure begins | Choose and rehearse a recovery approach that fits the maximum acceptable interruption. **Concrete situation (illustrative):** The service must recover within thirty minutes. → Rehearse detection, restore, and verification as one timeline. → Compare the measured total with the RTO. |
| RPO | The time range of data you allow yourself to go back in time and lose during the recovery process. | Set backup and replication requirements according to how much recent progress may be lost. **Concrete situation (illustrative):** The recovery requirement permits five minutes of lost progress. → Inspect backup and replication recovery points. → Verify the actual recoverable point meets that limit. |
| Capacity | Amount of requests, storage, and processing that the system can handle | Check whether expected demand and failure scenarios fit the available processing and storage. **Concrete situation (illustrative):** An upcoming campaign may double traffic. → Test representative load and downstream limits. → Record the highest sustained useful throughput meeting latency requirements. |
