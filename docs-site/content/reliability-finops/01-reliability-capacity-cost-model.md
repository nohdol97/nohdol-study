...
- **Reliability**: The property of a system to continue performing its expected function for a required period of time. **Why it matters / when to use it:** Evaluate sustained successful operation across failures, changes, and recovery periods. **Concrete situation (illustrative):** A service passes a short test but repeatedly fails during deployment. → Include change and recovery periods in reliability measurement. → Compare sustained successful outcomes.
- **SLI/SLO**: How to measure real user results and what the goals are for those measurements. **Why it matters / when to use it:** Connect measured user outcomes to an agreed target when prioritizing reliability work. **Concrete situation (illustrative):** Teams optimize uptime while customers complain about rejected orders. → Define an order-outcome SLI and target. → Check the agreed SLO over its full window.
- **failure mode**: A scenario that specifically describes what can fail and how. **Why it matters / when to use it:** Design focused failure drills and controls for a specific way the system can break. **Concrete situation (illustrative):** A worker can stop after output commit but before acknowledgment. → Add that interruption to the failure matrix. → Verify replay preserves the intended result.
- **Restore/recovery**: The process of making data and services usable again using a backup or remaining system. **Why it matters / when to use it:** Prove that retained data and replacement components can actually restore usable service. **Concrete situation (illustrative):** A backup exists but its application compatibility is unknown. → Restore into an isolated environment. → Check schema, data, permissions, and a complete user operation.
- **Capacity margin**: Processing capacity reserved to withstand surges beyond normal usage or some failures. **Why it matters / when to use it:** Absorb demand spikes or lost replicas without immediately exhausting all remaining capacity. **Concrete situation (illustrative):** A replica failure leaves the survivors at full utilization. → Reserve capacity for the planned failure case. → Verify latency remains acceptable with one replica unavailable.
- **FinOps**: An operating method that measures and improves cloud costs by linking them with technology and business ownership. **Why it matters / when to use it:** Connect infrastructure spending to accountable owners and meaningful business outcomes. **Concrete situation (illustrative):** Cloud spending rises without more successful orders. → Attribute costs to workload owners and useful outcomes. → Compare cost per valid order before and after changes.
