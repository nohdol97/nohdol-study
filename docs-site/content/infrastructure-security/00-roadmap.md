...
|---|---|---|
| identity | Information indicating who the person or program is that sent the request | Attribute actions to the requesting person or workload before applying permissions. **Concrete situation (illustrative):** An audit cannot distinguish two background jobs. → Give each workload an attributable identity. → Check action records identify the actual requester. |
| authentication | The process of verifying that the claimed identity is correct | Reject impersonation before treating a claimed identity as established. **Concrete situation (illustrative):** A request claims to be an administrator. → Verify its credential with the trusted authentication mechanism. → Reject the unverified claim before evaluating privileges. |
| authorization | The process of determining whether a confirmed identity can perform a specific task | Prevent an authenticated identity from performing actions outside its allowed scope. **Concrete situation (illustrative):** A logged-in user requests another tenant's report. → Evaluate resource-scoped authorization. → Confirm the request is denied despite successful login. |
| policy | Rules that state which actions will be allowed or denied under what conditions | Make access decisions repeatable and reviewable instead of relying on ad hoc exceptions. **Concrete situation (illustrative):** A report should be readable only by its owning team. → Express ownership and allowed actions in policy. → Test both permitted and forbidden callers. |
| secret information | Values ​​that, if exposed, may allow others to use the privileges | Recognize values that require restricted access because possession may enable privileged actions. **Concrete situation (illustrative):** A build log accidentally contains a credential. → Restrict exposure and replace the affected secret through the approved process. → Confirm the old credential no longer works. |
| artifact | Files resulting from build, such as container image or package to be deployed | Deploy and verify a specific build result rather than an ambiguous source or mutable tag. **Concrete situation (illustrative):** Staging passed, but production received a different package. → Compare artifact identities across promotion steps. → Deploy the exact reviewed build and verify its identity. |
