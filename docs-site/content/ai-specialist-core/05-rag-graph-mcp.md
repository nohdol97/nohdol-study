...
| question | suitable route | necessary guard |
|---|---|---|
...
Separate ANN recall from relevance recall. ANN recall compares approximate neighbors with exact neighbors under the same vector metric; relevance recall asks whether a human-relevant document was retrieved. A perfect nearest-neighbor search can still return irrelevant material. Apply access checks before context construction and test cross-tenant queries, deleted sources, and stale index entries. A row limit on generated SQL bounds returned rows, not necessarily scanned data or runtime; enforce read-only credentials, allowed objects/functions, and a query timeout separately.
