...
|---|---|---|
| token | The tokenizer divides the string into integer units of the model vocabulary. | Represent text in discrete model inputs and measure context length and generation work. **Concrete situation (illustrative):** A long prompt exceeds the model's context allowance. → Count tokens with the matching tokenizer. → Reduce input while preserving required evidence and instructions. |
| embedding | Expression of token ID converted into learnable vector | Turn discrete token IDs into learnable numeric inputs before contextual model computation. **Concrete situation (illustrative):** A language model receives token IDs that have no useful numeric distance by themselves. → Map IDs through the learned embedding table before contextual computation. → Inspect tensor shapes and distinguish token embeddings from later contextual states. |
| causal mask | Restrictions that prevent current location from seeing future tokens | Prevent next-token training from accessing future tokens unavailable during generation. **Concrete situation (illustrative):** Training predicts a token while accidentally seeing later text. → Apply the intended causal attention mask. → Verify future positions cannot influence that prediction. |
| attention | Calculation of weighted sum of values ​​based on query and key relationship | Combine relevant context positions when computing a token's representation. **Concrete situation (illustrative):** A token's interpretation depends on earlier context. → Inspect how attention combines eligible positions. → Compare behavior when the relevant context changes. |
| prefill / decode | Step of processing input tokens all at once / Step of sequentially generating the next token | Separate input processing from sequential generation when diagnosing latency and serving capacity. **Concrete situation (illustrative):** Long prompts delay the first token while long answers occupy workers afterward. → Measure prefill and decode separately. → Tune against the actual latency bottleneck. |
| KV cache | Memory that reuses already calculated past key·values ​​during decoding | Avoid recalculating past attention keys and values for every generated token, at a memory cost. **Concrete situation (illustrative):** Long conversations exhaust serving memory. → Budget KV cache alongside model weights and active requests. → Verify admission limits and generation latency under load. |
