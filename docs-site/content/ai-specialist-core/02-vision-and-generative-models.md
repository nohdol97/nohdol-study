...
|---|---|---|
| convolution | An operation to find local patterns by sharing a small kernel throughout the space. | Learn local spatial patterns with shared weights instead of separate parameters at every location. **Concrete situation (illustrative):** A defect can appear anywhere in a product image. → Learn local patterns with shared convolution kernels. → Test held-out images with varied defect positions. |
| residual | Connections that help the learning path of a deep network by adding input to the transformation result. | Help deep networks preserve information and train through shorter additive paths. **Concrete situation (illustrative):** A deeper network becomes difficult to optimize. → Evaluate residual connections within a controlled architecture comparison. → Compare training behavior and held-out performance. |
| patch embedding | An expression that divides an image into patches and changes it like a token sequence. | Convert image regions into sequence inputs that a transformer can process. **Concrete situation (illustrative):** A vision transformer must consume an image as a sequence. → Convert patches through its specified embedding path. → Verify patch order, shapes, and preprocessing consistency. |
| detection | The problem of predicting the class and location set of an object | Locate individual objects when knowing only the image's overall class is insufficient. **Concrete situation (illustrative):** A robot needs the positions of several boxes, not just an image label. → Evaluate an object detector. → Check localization and missed objects on relevant scenes. |
| segmentation | Problem predicting the class of each pixel or area | Identify precise occupied regions when bounding boxes cannot express the required shape. **Concrete situation (illustrative):** A defect's exact area matters more than its bounding box. → Evaluate segmentation masks. → Compare boundary errors and task-relevant region coverage. |
| latent | Internal representation more compressed than observed data | Perform learning or generation in a compact representation while checking which details are retained. **Concrete situation (illustrative):** A generative pipeline processes a compressed image representation. → Inspect the latent encoding and decoding stages. → Check reconstructed details and task quality. |
