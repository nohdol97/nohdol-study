...
|---|---|---|
| pruning | How to increase sparsity by removing some of the weight·channel·block or setting it to 0 | Explore smaller models by removing low-value components, then measure accuracy and actual runtime gains. **Concrete situation (illustrative):** A model is too large for the target device. → Prune selected structure and retune if required. → Measure target latency and accuracy, not parameter count alone. |
| quantization | How to approximate real numbers with lower bit integer/floating point representations | Reduce model storage and arithmetic cost when the target runtime supports lower precision adequately. **Concrete situation (illustrative):** A full-precision model exceeds device memory. → Test supported lower-precision quantization. → Compare memory, runtime speed, and representative prediction errors. |
| distillation | How to use the output/feature of a large teacher as a learning signal for a small student | Transfer useful teacher behavior to a smaller model when serving the teacher is too costly. **Concrete situation (illustrative):** A large teacher is accurate but too costly to serve. → Train a smaller student using approved teacher signals. → Evaluate the student's independent task and runtime results. |
| calibration | The process of adjusting the quantization range or confidence from representative data | Choose quantization ranges or assess confidence using representative data before deployment. **Concrete situation (illustrative):** A quantized model works on easy images but fails on dark scenes. → Revisit representative calibration data and ranges. → Evaluate dark and ordinary scenes separately. |
| structured sparsity | Scarcity of units that are easy for hardware to use, such as channel·block | Align removed model structure with patterns the target hardware can accelerate. **Concrete situation (illustrative):** Many weights are zero but inference is not faster. → Check the hardware's supported sparsity patterns. → Benchmark a compatible structured representation. |
| target artifact | A bundle of deployments with fixed runtime·precision·operator·device conditions as well as model | Reproduce deployment behavior by fixing the model together with its execution environment. **Concrete situation (illustrative):** The same model behaves differently on two runtimes. → Pin the full target artifact, including precision and operators. → Reproduce quality and latency on the intended device. |
