...
- **render**: This is the process of creating the final YAML that Kubernetes will receive by inserting values ​​into the template. **Why it matters / when to use it:** Inspect the actual manifests before installation to catch unintended generated configuration. **Concrete situation (illustrative):** A chart appears correct but installs an unexpected image. → Render it with the exact deployment values. → Inspect the generated image reference before installation.
- **desired state**: A desired state declared in Git or a configuration file that “this is how it should be”. **Why it matters / when to use it:** Let controllers compare the target with reality and repair deviations repeatedly. **Concrete situation (illustrative):** A Deployment requests three replicas but only two are ready. → Compare its desired and observed state. → Watch whether reconciliation restores three healthy replicas.
- **live state**: The status of resources that actually exist in the current cluster. **Why it matters / when to use it:** Compare actual objects with declared intent before claiming a deployment converged. **Concrete situation (illustrative):** Git declares four replicas while the cluster reports two. → Inspect live objects and controller conditions. → Explain why the observed state has not converged.
- **sync**: This is an operation to create, change, and delete resources to reduce the difference between the desired state and the live state. **Why it matters / when to use it:** Bring the cluster toward reviewed configuration after checking which changes reconciliation will make. **Concrete situation (illustrative):** A reviewed configuration is committed but not active. → Inspect synchronization status and proposed effects. → Verify intended objects change and requests remain healthy.
- **drift**: A phenomenon in which the desired state and the actual state differ. **Why it matters / when to use it:** Detect differences between reviewed Git intent and live objects before deciding how to reconcile them. **Concrete situation (illustrative):** Someone edits a live Kubernetes object after its Git configuration was reviewed. → Compare the live object with the declared Git state. → Reconcile the difference and verify the intended value is restored.
- **CRD**: A definition that adds a new type of resource to Kubernetes. **Why it matters / when to use it:** Represent a domain-specific object through Kubernetes' declarative API and controller workflows. **Concrete situation (illustrative):** A controller needs to manage a company-specific workload type. → Define and validate an appropriate CRD. → Check that accepted objects are reconciled as intended.
