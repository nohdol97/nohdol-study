...
|---|---|---|
| HBM / VRAM | High-bandwidth memory where GPU stores model·activation·KV cache | Budget accelerator memory for weights, activations, and growing request state before admitting work. **Concrete situation (illustrative):** Adding longer requests causes GPU memory failures. → Account for weights, activations, and KV cache. → Test the actual maximum admitted workload. |
| data parallel | How model replicas process different batches and synchronize gradients | Scale training across replicated models when the model fits each worker's memory. **Concrete situation (illustrative):** A model fits one GPU but training takes too long. → Evaluate data-parallel replicas with synchronized updates. → Measure throughput and convergence against the single-worker baseline. |
| tensor / pipeline parallel | A method of dividing the calculations and layers of one model into multiple devices | Distribute a model that is too large or costly for one device, accounting for communication overhead. **Concrete situation (illustrative):** The model cannot fit on one device. → Partition supported computation or layers across devices. → Measure communication, memory balance, and training correctness. |
| collective | Communication performed by multiple GPUs together, such as AllReduce and AllGather | Coordinate distributed tensor exchange so workers can produce consistent training updates. **Concrete situation (illustrative):** Distributed training waits during gradient exchange. → Inspect collective timing and worker participation. → Identify the slow communication path or straggler. |
| continuous batching | Scheduling to exclude completed requests at each decode step and add new requests to the batch | Use freed serving slots promptly as requests finish instead of waiting for a fixed batch to drain. **Concrete situation (illustrative):** Short generations finish while long ones keep a static batch occupied. → Evaluate continuous batching. → Compare completed throughput and per-request latency under mixed lengths. |
| MFU | Utilization perspective that compares the effective model calculation amount to the maximum hardware calculation amount | Check how effectively a training workload turns hardware capacity into useful model computation. **Concrete situation (illustrative):** Expensive GPUs show high activity but little useful training progress. → Compare effective model computation with hardware capacity. → Investigate communication and input stalls behind low MFU. |
