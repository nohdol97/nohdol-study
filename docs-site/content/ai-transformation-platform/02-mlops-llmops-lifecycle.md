...
|---|---|---|
| lineage | The relationship between which input, execution, and parent artifact the output came from | Trace an output to its inputs and runs when reproducing results or investigating changes. **Concrete situation (illustrative):** A deployed model's result differs from a saved evaluation. → Trace both artifacts to their data and runs. → Identify the changed input or configuration. |
| registry | Source of truth that manages versioned artifacts and alias/metadata | Select a reviewed artifact version and record which version an alias or deployment refers to. **Concrete situation (illustrative):** A production alias unexpectedly points to a new model. → Inspect registry versions and promotion records. → Verify the deployed identity and authorized transition. |
| pipeline | The input, processing, learning, evaluation, and packaging steps are made into reproducible DAG. | Make multi-step learning workflows repeatable and inspectable instead of relying on notebook order. **Concrete situation (illustrative):** A notebook succeeds only when cells run in a remembered order. → Encode input and processing dependencies as a pipeline. → Reproduce the output from a clean run. |
| eval suite | Evaluation bundle with fixed dataset, metric, judge·policy, and threshold | Compare candidates under the same versioned examples and acceptance criteria. **Concrete situation (illustrative):** Two model candidates are scored with different examples. → Freeze the evaluation suite and criteria. → Compare both candidates under identical versions. |
| drift | Data·prediction·A phenomenon in which the distribution of work results differs from the baseline | Detect distribution changes that may invalidate a model's earlier evaluation or monitoring baseline. **Concrete situation (illustrative):** A model sees a new customer population after a product launch. → Compare input and outcome distributions with baseline. → Re-evaluate quality before blaming infrastructure. |
| promotion | Decision to allow a candidate to pass through the gate and move to the limited serving stage | Prevent an unqualified candidate from reaching serving simply because its build succeeded. **Concrete situation (illustrative):** A build succeeds despite failing the safety evaluation. → Keep promotion conditional on all required gates. → Verify the failed candidate cannot reach serving. |
