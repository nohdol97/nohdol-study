...
|---|---|---|
| AI infrastructure | Foundation for operating GPU·network·storage·scheduler·serving runtime | Provide the shared execution capacity on which training and inference reliability depend. **Concrete situation (illustrative):** GPU workers are idle while input data arrives slowly. → Inspect storage, network, and scheduler dependencies. → Measure the bottleneck before buying more compute. |
| MLOps / LLMOps | A system that reproduces dataset·model·prompt·index·evaluation and deployment history | Reproduce and govern changes across data, models, prompts, evaluation, and deployment. **Concrete situation (illustrative):** A model regression cannot be tied to a dataset or prompt revision. → Record the complete training and deployment lineage. → Reproduce the candidate under its original inputs. |
| continuous training | The process of repeatedly generating learning candidates based on new data and criteria | Generate updated candidates as data changes while keeping evaluation and promotion gates. **Concrete situation (illustrative):** New data arrives daily and may improve the model. → Train candidates through the existing gated workflow. → Promote only after reproducible evaluation, not on schedule alone. |
| model serving | A layer where multiple requests share model inference with safe latency and capacity | Share inference capacity across clients under explicit latency and overload policies. **Concrete situation (illustrative):** Concurrent generation requests exhaust the serving worker. → Apply measured admission and batching policies. → Check tail latency, successful completions, and rejection behavior. |
| capability bundle | A deployment unit that fixes not only the model but also the prompt·tool·policy·workflow and runtime together. | Avoid deploying a model with mismatched prompts, tools, policies, or runtime settings. **Concrete situation (illustrative):** A tested model is deployed with an untested prompt and tool version. → Promote the reviewed capability bundle together. → Verify all runtime components match that bundle. |
| receipt | Verification record of who implemented what and with what inputs, policies, and results | Reconstruct what actually executed instead of treating a plan or generated answer as execution proof. **Concrete situation (illustrative):** A report says a change ran, but only a plan exists. → Require an execution receipt with inputs and results. → Distinguish planned work from observed effects. |
