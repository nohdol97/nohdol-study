...
|---|---|---|
| expand-contract | First, add expressions that will be used together in the old and new versions, and then remove old expressions after conversion. | Change schemas or APIs gradually while old and new versions coexist. **Concrete situation (illustrative):** A column rename would break an older deployed client. → Add the compatible form, migrate readers, then remove the old form. → Verify mixed-version behavior at each stage. |
| contract test | A test to check whether the actual implementation adheres to the requests and responses agreed upon by the provider and consumer. | Detect incompatible provider changes before they break dependent consumers. **Concrete situation (illustrative):** A provider changes a required response field. → Run the consumer's contract tests before release. → Confirm the incompatible candidate is rejected. |
| shadow read | Validation that compares the results of a new path to existing results without writing them to the user. | Compare a new read path with the established one before relying on it for user responses. **Concrete situation (illustrative):** A new database query should replace an established read path. → Compare shadow results without changing user output. → Investigate mismatches before switching traffic. |
| canary | Steps to expose new changes to only some traffic·tenant·resources | Observe a release on a limited population before exposing all users to its risk. **Concrete situation (illustrative):** A new release may increase checkout errors. → Expose a constrained population to the candidate. → Compare matched outcomes before expanding traffic. |
| rollback | Reverting an execution artifact to a previous revision | Restore a known deployment revision when a new release harms behavior and reversal remains compatible. **Concrete situation (illustrative):** A new release causes errors with an unchanged compatible schema. → Restore the reviewed earlier artifact. → Verify user requests, not only rollout status. |
| roll forward | Forward deployment of a modified version when a simple rollback is risky due to data or external effects. | Repair behavior with a compatible new version when reversing the old change would worsen state. **Concrete situation (illustrative):** A release changed data in a way the old binary cannot read. → Prepare a compatible forward fix. → Verify both data interpretation and user recovery. |
