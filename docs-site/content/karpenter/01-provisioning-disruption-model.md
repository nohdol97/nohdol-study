...
- **provisioning**: The process of selecting and preparing new compute resources to run workloads. **Why it matters / when to use it:** Supply missing compute capacity when eligible workloads cannot fit existing Nodes. **Concrete situation (illustrative):** New Pods cannot fit existing Nodes. → Inspect eligible provisioning options and startup delays. → Confirm new capacity actually makes the Pods ready.
- **requirement**: Conditions such as CPU architecture, zone, and capacity type that Pod or NodePool allows or requires. **Why it matters / when to use it:** Prevent placement on incompatible hardware, zones, or purchasing options. **Concrete situation (illustrative):** An image requires one CPU architecture but available Nodes use another. → Align workload and Node requirements. → Verify image execution on the selected hardware.
- **capacity type**: A value that classifies EC2 by purchase method such as On-Demand or Spot. **Why it matters / when to use it:** Balance workload interruption tolerance and purchasing constraints when selecting compute capacity. **Concrete situation (illustrative):** A batch workload can tolerate interruptions but an API cannot tolerate the same pattern. → Compare capacity types against each recovery requirement. → Rehearse interruption handling.
- **consolidation**: An operation to reduce unnecessary nodes when the workload can be safely moved to fewer nodes. **Why it matters / when to use it:** Reduce unused capacity cost after proving the remaining placement can sustain the workload. **Concrete situation (illustrative):** Several Nodes stay mostly idle overnight. → Evaluate a consolidation plan with placement constraints. → Check remaining capacity and disruption before accepting removal.
- **PDB**: A Kubernetes policy that limits the number of Pods that can become unavailable simultaneously during a voluntary outage. **Why it matters / when to use it:** Limit concurrent voluntary evictions so planned maintenance respects the workload's availability needs. **Concrete situation (illustrative):** Maintenance tries to evict too many replicas together. → Inspect the workload's PDB and healthy replicas. → Verify allowed voluntary disruption matches the availability requirement.
- **graceful termination**: A procedure that terminates a process by giving it time to organize its ongoing work. **Why it matters / when to use it:** Let in-flight work finish or checkpoint before the process is stopped. **Concrete situation (illustrative):** Deployment terminates workers while they handle requests. → Stop accepting new work and allow bounded draining. → Check completed, retried, and interrupted requests separately.
