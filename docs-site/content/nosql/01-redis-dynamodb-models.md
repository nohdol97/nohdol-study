...
- **source of truth**: This is the original data that will ultimately be judged correct when other copies are incorrect. **Why it matters / when to use it:** Resolve disagreements between caches, replicas, and other copies using an authoritative record. **Concrete situation (illustrative):** The cache and database disagree on an order status. → Consult the declared source of truth. → Repair the derived copy and verify the consumer view.
- **cache**: A copy of the result that is temporarily stored nearby to reduce slow lookup of the original. **Why it matters / when to use it:** Reduce repeated origin work for results that can be safely reused for a defined period. **Concrete situation (illustrative):** Many users repeatedly request the same public catalog. → Cache the reusable response with a freshness policy. → Compare origin load and returned data age.
- **eviction**: This is an operation to remove some keys according to a set policy when memory is insufficient. **Why it matters / when to use it:** Keep a memory-limited cache operating by choosing which entries can be discarded. **Concrete situation (illustrative):** Cache memory fills while new keys keep arriving. → Review and test the eviction policy. → Check miss rate and whether required data is stored elsewhere.
- **persistence**: This method leaves data on a separate storage device so that it can be recovered even if the process is terminated or restarted. **Why it matters / when to use it:** Recover required state after restart instead of relying solely on process memory. **Concrete situation (illustrative):** A cache-backed service restarts unexpectedly. → Restore according to its configured persistence contract. → Measure which accepted updates survived and whether loss is allowed.
- **partition**: A unit that divides a lot of data into several storage areas. **Why it matters / when to use it:** Spread storage or processing work while defining the resulting ordering and access boundaries. **Concrete situation (illustrative):** One data shard is busy while others are idle. → Inspect partition placement and key distribution. → Compare load after a compatible redistribution.
- **secondary index**: An additional index maintained to find data using conditions other than the primary key. **Why it matters / when to use it:** Support additional query patterns without scanning all items by the primary key layout. **Concrete situation (illustrative):** A new query finds orders by customer rather than order ID. → Evaluate a suitable secondary index. → Compare query work and index maintenance cost.
