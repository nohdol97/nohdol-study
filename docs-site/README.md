...
All 94 articles support Korean, English, and paired reading. Paired reading is the default: Korean comes first, followed by visually secondary English that can be collapsed individually or together. Titles follow the same order. Code, commands, output fixtures, and diagrams appear once. Search indexes both languages. The header's interface language remains a separate preference and localizes navigation, titles, summaries, search, and diagram controls. Both preferences survive reload when storage is available and work in memory otherwise. See the [bilingual reading criteria](../docs/specs/2026-09-11-bilingual-reading.md).

Open an underlined term or the chapter's terminology list for bilingual definitions, purpose, and concrete situations. Every entry separates its illustrative example into Situation, Apply, and Check steps. Chapter definitions are reused within their topic; CDC, dbt, and other core technologies also compare adjacent roles. Context links return to the source chapter, and core entries link to official documentation where available. Explanations are teaching synthesis, not additional evidence or recorded lab results.
...
Append ` **Concrete situation (illustrative):** ` to each purpose cell or list suffix, followed by three parts separated by ` → `: a specific situation, how to apply or respond to the concept, and what to verify. Use ` **구체적인 상황(가상 예시):** ` in Korean. The extractor keeps purpose and scenario separate; the dialog labels each step and pairs its two languages. Core entries use the same three-part format in `exampleEn` and `exampleKo`. Missing, one-sided, or malformed scenarios reject publication. Structural checks establish coverage, not factual or pedagogical quality: review the scenario against the chapter, preserve domain-specific meanings, and label invented circumstances as illustrative.
