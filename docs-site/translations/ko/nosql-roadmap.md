...
|---|---|---|
| 키(key) | 데이터를 다시 찾을 때 사용하는 고유한 이름 | 무관한 값을 모두 훑지 않고 원하는 항목을 조회·갱신할 때 쓴다. |
| 값(value) | 키에 연결해 저장하는 실제 데이터 | 키로 다시 조회할 실제 애플리케이션 정보를 담는 데 쓴다. |
| TTL | 데이터가 자동으로 사라질 때까지 남은 시간 | 세션·캐시 같은 임시 데이터에 수명 정책을 적용할 때 쓴다. |
| 메모리(memory) | 매우 빠르지만 전원 장애 뒤 보존 방법을 따로 고려해야 하는 저장 공간 | 자주 읽는 데이터를 빠르게 제공할 때 쓰며 영속성과 용량은 별도로 설계한다. |
| 파티션 키(partition key) | DynamoDB가 데이터를 어느 저장 구역에 둘지 결정할 때 쓰는 키 | DynamoDB 접근을 분산하고 소수의 배치 키에 부하가 집중되지 않도록 설계할 때 쓴다. |
| 일관성(consistency) | 쓰기 직후 읽었을 때 최신 값이 보이는지에 관한 보장 | 오래된 값이 애플리케이션 판단을 바꿀 수 있을 때 필요한 읽기 가시성을 정한다. |
