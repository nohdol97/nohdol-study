...
|---|---|---|
| Pod | Kubernetes가 함께 실행하고 관리하는 container 묶음 | 밀접하게 연결된 컨테이너를 같은 곳에 배치하고 함께 관리할 때 쓴다. **구체적인 상황(가상 예시):** API와 보조 프로세스가 로컬 파일과 수명을 공유해야 한다. → 밀접한 컨테이너를 한 Pod에 배치한다. → 공유 볼륨 접근과 재시작 동작을 확인한다. |
