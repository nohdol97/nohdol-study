...
|---|---|---|
| 관리형 계산(managed compute) | 공급자가 준비와 운영의 일부를 맡아 주는 계산 자원이다. | 워크로드 정확성·비용에 대한 책임은 유지하면서 인프라 운영의 일부를 위임한다. **구체적인 상황(가상 예시):** 팀이 워커 호스트를 유지보수하지 않고 데이터 잡을 실행하려 한다. → 확장·런타임·비용 제약을 정해 관리형 컴퓨팅을 선택한다. → 작업 동작과 여전히 팀이 맡아야 할 운영 책임을 확인한다. |
| Unity Catalog | Databricks에서 지원되는 데이터와 자산을 찾고 관리하는 계층이다. | 지원되는 Databricks 워크로드에서 자산 검색과 접근 통제를 함께 관리한다. **구체적인 상황(가상 예시):** Databricks 팀들이 공유 데이터의 권한과 검색을 일관되게 관리해야 한다. → 지원되는 자산을 Unity Catalog로 등록·관리한다. → 의도한 범위에서 실제 사용자·서비스 신원으로 접근을 시험한다. |
| Lakeflow | Databricks에서 데이터 수집, 파이프라인 구성, 작업 조정을 제공하는 기능 모음이다. | Databricks 데이터 엔지니어링 도구에서 수집·변환·작업 실행을 조정한다. **구체적인 상황(가상 예시):** Databricks 파이프라인에 수집·변환·정기 실행이 필요하다. → 요구별로 적절한 Lakeflow 기능을 연결한다. → 선택한 구성 요소의 의존성·재시도·품질 결과를 확인한다. |
| 동적 테이블(dynamic table) | 쿼리로 정의하고 갱신을 통해 결과를 유지하는 Snowflake 테이블이다. | 신선도 요구를 명시하고 관리되는 갱신 동작으로 쿼리가 정의한 분석 결과를 유지한다. **구체적인 상황(가상 예시):** Snowflake 팀이 파생 테이블의 갱신 로직을 반복 관리한다. → 지원되는 동적 테이블과 목표 지연을 정의한다. → 대표 변경에서 실제 최신성·갱신 동작·컴퓨팅 비용을 관찰한다. |
| DMF | Snowflake의 데이터 메트릭 함수다. 결측값 수처럼 데이터의 특정 속성을 측정한다. | 데이터 속성을 반복 측정해 기록된 결과로 품질 기대를 평가한다. **구체적인 상황(가상 예시):** Snowflake 테이블의 결측 등 품질 상태를 관찰해야 한다. → 적절한 데이터 메트릭 함수를 연결하고 지원되는 모니터링을 설정한다. → 측정값과 별도로 정의한 기대 조건·경보를 확인한다. |
| Warehouse | Snowflake의 가상 웨어하우스는 지원되는 작업을 실행할 계산 자원이다. 저장된 데이터 자체를 뜻하지 않는다. | 동시성·지출을 관리하면서 워크로드용 Snowflake 계산 자원을 할당·조정한다. **구체적인 상황(가상 예시):** 무거운 변환이 Snowflake 대화형 쿼리와 경쟁한다. → 가상 웨어하우스 분리와 적절한 크기를 검토한다. → 의도한 동시 실행에서 지연과 크레딧 사용을 비교한다. |
