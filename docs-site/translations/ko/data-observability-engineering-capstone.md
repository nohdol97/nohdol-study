...
`publish`는 새 결과를 반환하기 전에 잘못된 값, 같은 ID의 내용 충돌, 예상 입력 누락을 검사한다. 실패하면 예외가 나고 기존 스냅샷은 바뀌지 않는다. `reconcile`이 먼저 받은 내용을 남기는 것은 충돌 조사용이다. 먼저 도착한 값을 정답으로 정한 것이 아니며 테스트는 두 도착 순서 모두 공개를 차단한다. 실제 갱신을 지원하려면 버전 순서와 수정 규칙을 추가해야 한다. 이 예제는 메모리에서 검사할 뿐 파일 저장이나 동시 공개는 구현하지 않는다.
...
산출물은 데이터 생성기, 스키마, 예상 입력·결과 목록, 배치 측정, 테이블 이력, 복구 절차다. 물리 배치의 근거와 업무 합계의 근거를 구분한다.

## 3단계: Kafka와 Spark

같은 주문 예제를 Kafka → Spark → 테이블 순서로 처리한다. Kafka 오프셋과 이벤트 ID를 각각 보관한다. 같은 이벤트 재전송, 같은 ID의 다른 내용, 순서가 바뀐 도착, 늦은 도착, 소비자 재시작을 하나씩 시험한다. 처음에는 로컬 예제와 같은 규칙으로 결과를 비교한다. 이벤트 시간이나 갱신을 추가할 때는 분산 구현과 정답 계산 예제에 같은 규칙을 함께 적용한다.
...
느린 stage 사고, HTTP는 정상인데 데이터가 오래된 사고, 독립적인 텔레메트리 장애를 제출한다. 복구를 시연하고 데이터 유실·신호 유실을 따로 대조한다.
...
| 늦은 이벤트 | 수락·수정·거부 건수와 하위 대조 |
| 스키마·단위 변경 | 사용자 호환성 결과와 버전 있는 전환 |
| 소비자·실행기 중단 | 진행 상태 재개와 올바른 공개 출력 |
| 체크포인트·메타데이터 유실 | 시간 측정·누락 범위 표시를 포함한 격리 복원·재생 |
| 예약 실행 누락 | SLO 분모 포함과 제한 백필 복구 |
| Collector·백엔드 장애 | 알려진 신호 적체·유실과 관측 범위 알림 |
| 계보 이벤트 누락 | 수집 누락 범위 표시와 영향 사용자 조사 검증 |
| 소스 접근 권한 취소 | 금지 검색·프롬프트·답·디버그 노출 없음 |
...
검사를 통과한 스냅샷에는 이벤트 두 개와 합계 350센트가 남는다. 같은 ID에 다른 금액을 넣거나, 입력을 빠뜨리거나, 음수·불리언 금액과 잘못된 형식의 입력을 넣으면 새 결과를 제공하기 전에 실패한다. 동작이 바뀌면 단언문이 이를 잡는다. 이후 Kafka·클라우드 복구, 신호 전송, AI 평가를 확인하려면 각 서비스를 실제로 실행한 기록이 따로 필요하다. 이 Python 예제로 확인한 범위와 구분한다.
...
서비스 연동 전에 장별 예제를 구성 요소 정답 기준으로 쓴다. 저장소 체크아웃에서 `python3 docs-site/labs/verify_data_course.py`를 실행하면 표시된 표준 라이브러리 예제 일곱 개의 출력과 Markdown을 비교한다. 윈도·재귀, 타입 거부, 사건 당시 기준의 분류, 품질 차원, 히스토그램 보간, 계보 탐색, 하이브리드 순위를 다룬다.
...
| 가져올 예제 | 연동 근거 |
|---|---|
| 윈도·SCD 경계 | 분산 조인이 키·사건 당시 기준의 분류·합계를 보존 |
...
| Kafka 중단·재균형 | 완료 작업까지만 진행 상태하고 재시도가 업무 효과 보존 |
| dbt 갱신·삭제 | 지원 변경 유형 모두에서 증분과 전체 재구성 일치 |
| 품질 측정 대상 | 소스 ID 누락·검사 부재가 설계대로 승인 차단 |
| OTel 전파 | 전송 헤더가 관계를 보존하고 신호 유실은 별도 집계 |
| 히스토그램·알림 계산 | 백엔드 결과가 예제 측정 대상·관측 범위 정책과 일치 |
| 계보·접근 | 알려진 후속 데이터셋을 찾고 실제 신원으로 금지 읽기 실패 |
| 클라우드 구현 | 로컬·관리형 출력, 정책, 복구, 비용 비교 |
| 검색·평가 | 허용 근거가 순위·프롬프트·답·트레이스까지 버전 유지 |

장별 로컬 예제가 통과하면 그다음 서비스 간 연결을 시험한다. 예를 들어 SDK 실습은 한 프로세스 안에서 컨텍스트를 주고받는 것을 확인한다. Kafka에 연결할 때는 헤더가 실제 메시지에 저장되고 다른 프로세스에서 제대로 읽히는지 추가로 확인해야 한다. 어떤 단계까지 실행했는지 포트폴리오에 구분해 적어야 다른 엔지니어가 같은 범위로 재현할 수 있다.
