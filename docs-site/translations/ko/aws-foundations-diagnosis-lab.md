...
가상으로 구성하고 민감 정보를 제거한 AWS CLI 예시다. 읽기 연습용이며 실제 계정을 조회해 만들지 않았다.
...
선택한 경로는 다음과 같이 표시될 수 있다. 이 조회가 NAT 대상이나 빈 목록을 반환할 수도 있다.
...
사용하려던 계정과 역할에 일치해야 신원 검토를 통과한다. 목록이 비었다고 권한 실패가 입증되는 것은 아니며, STS 호출 성공이 EC2 조회 권한을 의미하지도 않는다. 경로 설정은 엔드포인트 도달 시험으로도 확인해야 한다.
