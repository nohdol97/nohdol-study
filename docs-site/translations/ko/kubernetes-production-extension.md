...
오프라인 Kustomize 렌더링에서 선택한 필드의 예상값이다.
...
이 출력은 검토를 위해 줄인 일부이며 그대로 적용할 완전한 매니페스트가 아니다. 렌더링한 이름, 복제본 네 개, 이미지 태그, 변경 없이 서로 일치하는 셀렉터가 오버레이와 맞아야 통과다. `kubectl diff`는 차이가 있으면 1을 반환하며 이는 렌더링 오류가 아니다. 서버 검증과 롤아웃에는 선택한 임시 클러스터가 필요하다. 오버레이를 적용했다면 `kubectl delete -k k8s/overlays/production`으로 해당 리소스를 제거한다.
...
[← 관측과 트러블슈팅](../../content/kubernetes/09-observability-and-troubleshooting.md) · [전체 로드맵으로 돌아가기](../../content/kubernetes/00-roadmap.md)
