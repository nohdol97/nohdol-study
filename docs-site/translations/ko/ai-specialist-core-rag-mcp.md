...
|---|---|---|
| chunk | 검색할 수 있도록 source를 나눈 단위와 metadata | 출처·권한 맥락을 유지하면서 다룰 수 있는 크기의 근거 구간을 검색한다. **구체적인 상황(가상 예시):** 검색된 문단에 정책 해석에 필요한 조건이 빠져 있다. → chunk 경계와 보존 메타데이터를 검토한다. → 검색 근거가 조건 전체를 담는지 확인한다. |
| embedding | 의미상 가까움을 계산하기 위한 vector 표현 | 정확한 단어 일치로 관련 근거를 찾기 어려울 때 벡터 공간에서 의미 후보를 비교한다. **구체적인 상황(가상 예시):** 자동차 보험 검색이 차량 보장이라는 문서를 놓친다. → 임베딩 기반 의미 후보를 비교한다. → 정답 예제로 관련성·권한을 평가한다. |
| ANN | 모든 vector를 정확히 비교하지 않고 가까운 후보를 빠르게 찾는 검색 | 큰 규모의 벡터 검색 작업량을 줄이되 근사 검색으로 잃는 재현율을 측정한다. **구체적인 상황(가상 예시):** 문서 전체의 정확한 벡터 비교가 너무 느리다. → 정답 질의 집합으로 ANN 인덱스를 시험한다. → 재현율·지연·권한 동작을 비교한다. |
| reranker | 빠른 후보 집합을 더 비싼 모델로 다시 정렬하는 단계 | 전체 문서 대신 작은 후보 집합에 더 정교한 관련성 모델의 비용을 쓴다. **구체적인 상황(가상 예시):** 1차 검색이 관련 문서를 찾지만 순서가 나쁘다. → 허용된 후보 집합을 재정렬한다. → 관련성 개선과 추가 지연을 비교한다. |
| grounding | 답변의 주장이 제공한 근거로 실제 지지되는 성질 | 답변의 주장이 그럴듯한 표현만이 아니라 권한이 허용된 근거로 뒷받침되는지 확인한다. **구체적인 상황(가상 예시):** 답변이 인용한 정책은 주장한 예외를 뒷받침하지 않는다. → 주장과 제공된 구간을 비교한다. → 근거 없는 주장을 제거하거나 불확실성을 표시한다. |
| tool | model이 호출을 제안할 수 있는 외부 기능이며 별도 권한이 필요한 경계 | 모델이 통제된 권한 경계를 통해 외부 정보·작업을 요청하도록 한다. **구체적인 상황(가상 예시):** 도우미가 티켓 상태 변경을 제안한다. → 도구의 권한·검사 경계를 통해 요청한다. → 허용된 작업과 기록된 결과를 확인한다. |
...
ANN 재현율과 관련성 재현율을 구분한다. ANN 재현율은 동일한 벡터 거리 척도에서 근사 이웃과 정확한 이웃을 비교하고, 관련성 재현율은 사람에게 관련 있는 문서를 검색했는지 묻는다. 최근접 이웃 검색이 완벽해도 무관한 자료를 반환할 수 있다. 컨텍스트 구성 전에 접근 권한을 확인하고 테넌트 간 질의, 삭제된 출처, 오래된 인덱스 항목을 시험한다. 생성 SQL의 행 제한은 반환 행 수를 제한할 뿐 읽는 데이터나 실행 시간을 반드시 제한하지는 않는다. 읽기 전용 자격 증명, 허용 오브젝트·함수, 쿼리 시간 제한을 별도로 적용한다.
