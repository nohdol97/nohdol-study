...
|---|---|---|
| chunk | 검색할 수 있도록 source를 나눈 단위와 metadata | 출처·권한 맥락을 유지하면서 다룰 수 있는 크기의 근거 구간을 검색한다. |
| embedding | 의미상 가까움을 계산하기 위한 vector 표현 | 정확한 단어 일치로 관련 근거를 찾기 어려울 때 벡터 공간에서 의미 후보를 비교한다. |
| ANN | 모든 vector를 정확히 비교하지 않고 가까운 후보를 빠르게 찾는 검색 | 큰 규모의 벡터 검색 작업량을 줄이되 근사 검색으로 잃는 재현율을 측정한다. |
| reranker | 빠른 후보 집합을 더 비싼 모델로 다시 정렬하는 단계 | 전체 문서 대신 작은 후보 집합에 더 정교한 관련성 모델의 비용을 쓴다. |
| grounding | 답변의 주장이 제공한 근거로 실제 지지되는 성질 | 답변의 주장이 그럴듯한 표현만이 아니라 권한이 허용된 근거로 뒷받침되는지 확인한다. |
| tool | model이 호출을 제안할 수 있는 외부 기능이며 별도 권한이 필요한 경계 | 모델이 통제된 권한 경계를 통해 외부 정보·작업을 요청하도록 한다. |
