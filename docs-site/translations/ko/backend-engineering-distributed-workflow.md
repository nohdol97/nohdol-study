...
| delivery | message가 broker와 consumer 사이에서 전달되는 규칙 | 수신을 완료와 동일시하지 않고 소비자가 처리해야 할 유실·중복 가능성을 정한다. **구체적인 상황(가상 예시):** 소비자가 재시작 뒤 같은 이벤트를 다시 받는다. → 전달 계약과 중복 처리 규칙을 적용한다. → 수신 횟수뿐 아니라 업무 효과를 확인한다. |
