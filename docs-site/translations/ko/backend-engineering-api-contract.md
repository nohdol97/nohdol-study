...
|---|---|---|
| resource | API가 식별하고 표현하는 대상 | 호출자가 API 계약을 통해 식별·조회·변경할 안정적인 대상을 정의할 때 쓴다. |
| safe | 호출자가 상태 변경을 요청하지 않는 method 성질 | 클라이언트·API를 설계할 때 읽기 의도의 HTTP 메서드와 변경 요청을 구분한다. |
| idempotent | 같은 요청을 반복해도 의도한 서버 효과가 한 번과 같은 성질 | 응답이 불확실할 때 요청을 반복해도 의도한 서버 효과를 유지할 수 있는 조건을 판단한다. |
| representation | resource의 현재 상태를 전송 가능한 형식으로 표현한 값 | 클라이언트가 정해진 응답 형식으로 자원의 상태를 읽고 교환하도록 한다. |
| problem detail | 기계가 읽을 수 있는 공통 오류 본문 형식 | 클라이언트가 오류를 처리·진단할 수 있도록 일관된 기계 판독용 실패 정보를 제공한다. |
| operation | 응답보다 오래 실행되는 한 번의 업무 작업과 그 상태 | 최초 HTTP 응답보다 오래 걸리는 작업의 진행 상태와 결과를 제공할 때 쓴다. |
