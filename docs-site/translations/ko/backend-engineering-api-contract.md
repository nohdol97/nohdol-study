...
|---|---|---|
| resource | API가 식별하고 표현하는 대상 | 호출자가 API 계약을 통해 식별·조회·변경할 안정적인 대상을 정의할 때 쓴다. **구체적인 상황(가상 예시):** 클라이언트가 수정할 주문을 안정적으로 식별하지 못한다. → 고정된 주문 자원과 API 작업을 정의한다. → 반복 조회가 같은 주문을 가리키는지 확인한다. |
| safe | 호출자가 상태 변경을 요청하지 않는 method 성질 | 클라이언트·API를 설계할 때 읽기 의도의 HTTP 메서드와 변경 요청을 구분한다. **구체적인 상황(가상 예시):** 브라우저가 GET 주소를 미리 읽는다. → 상태 변경 요청을 safe 메서드 의미와 분리한다. → 미리 읽기가 구매를 발생시키지 않는지 확인한다. |
| idempotent | 같은 요청을 반복해도 의도한 서버 효과가 한 번과 같은 성질 | 응답이 불확실할 때 요청을 반복해도 의도한 서버 효과를 유지할 수 있는 조건을 판단한다. **구체적인 상황(가상 예시):** 클라이언트가 응답 유실 후 PUT을 반복한다. → 메서드의 멱등 효과를 유지한다. → 한 번과 반복 요청 뒤의 자원 상태를 비교한다. |
| representation | resource의 현재 상태를 전송 가능한 형식으로 표현한 값 | 클라이언트가 정해진 응답 형식으로 자원의 상태를 읽고 교환하도록 한다. **구체적인 상황(가상 예시):** 클라이언트가 DB 테이블에 접근하지 않고 주문 상태를 알아야 한다. → 주문 자원의 문서화된 표현을 응답한다. → 클라이언트 관점에서 필드 의미와 호환성을 시험한다. |
| problem detail | 기계가 읽을 수 있는 공통 오류 본문 형식 | 클라이언트가 오류를 처리·진단할 수 있도록 일관된 기계 판독용 실패 정보를 제공한다. **구체적인 상황(가상 예시):** 클라이언트가 제각각인 오류 형식을 해석한다. → 문서화된 문제 상세 계약을 적용한다. → 실패별 상태·유형·대응 필드를 확인한다. |
| operation | 응답보다 오래 실행되는 한 번의 업무 작업과 그 상태 | 최초 HTTP 응답보다 오래 걸리는 작업의 진행 상태와 결과를 제공할 때 쓴다. **구체적인 상황(가상 예시):** 보고서 내보내기가 최초 HTTP 요청의 대기 시간보다 오래 걸린다. → 문서화된 상태 조회 위치와 작업 식별자를 반환한다. → 진행·완료·실패·결과 조회 동작을 확인한다. |
