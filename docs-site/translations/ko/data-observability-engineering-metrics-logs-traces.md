...
| 용어 | 의미 |
|---|---|
| 카운터(counter) | 생산자 재시작 시 초기화될 수 있는 누적 건수 |
| 게이지(gauge) | 증가·감소할 수 있는 현재 값 |
| 히스토그램(histogram) | 관측값 분포의 표현 |
| 서머리(summary) | 설정 시 클라이언트가 분위수를 계산하는 관측 형식 |
| 라벨 카디널리티 | 시계열을 만드는 서로 다른 라벨 조합의 수 |
| 소진율(burn rate) | 관측 실패 비율을 SLO 허용 실패 비율로 나눈 값 |
