...
|---|---|---|
| 드라이버(driver) | 애플리케이션과 작업을 조정하는 프로세스 | 계획·조정 단계의 병목을 실행기의 태스크 처리와 구분해 조사한다. |
| 실행기(executor) | 태스크를 실행하고 실행 데이터를 보유하는 워커 프로세스 | 파티션 태스크를 실행하고 튜닝 시 워커의 메모리·디스크·CPU 한계를 찾는다. |
| Job / stage / task | 액션으로 시작된 작업 / 스케줄링 단계 / 파티션 하나의 작업 | 느린 작업을 스케줄링 단계와 지연을 일으키는 특정 파티션에 연결한다. |
| 셔플(shuffle) | 흔히 네트워크·디스크를 거쳐 레코드를 파티션 사이에 재분배하는 것 | 조인·집계를 위해 관련 키를 모으되 네트워크·디스크 작업량을 고려한다. |
| 쏠림(skew) | 작업·데이터 분포가 불균등해 느린 파티션이 생기는 현상 | 자원을 모두 늘리는 대신 유독 느린 작업을 개선하도록 불균등한 파티션을 찾는다. |
| 디스크 넘김(spill) | 중간 데이터를 메모리에 유지할 수 없어 디스크로 옮기는 것 | 사용 가능한 메모리를 넘어서도 작업이 진행되는 방식과 디스크 입출력 증가 이유를 이해한다. |
| AQE | 적응형 쿼리 실행. 런타임 통계로 지원되는 실행 계획 선택을 바꾸는 기능 | 초기 추정이 부정확할 때 실제 실행 측정으로 지원되는 쿼리 계획 선택을 조정한다. |
