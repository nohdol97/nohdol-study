...
| 용어 | 의미 |
|---|---|
| 드라이버(driver) | 애플리케이션과 작업을 조정하는 프로세스 |
| 실행기(executor) | 태스크를 실행하고 실행 데이터를 보유하는 워커 프로세스 |
| Job / stage / task | 액션으로 시작된 작업 / 스케줄링 단계 / 파티션 하나의 작업 |
| 셔플(shuffle) | 흔히 네트워크·디스크를 거쳐 레코드를 파티션 사이에 재분배하는 것 |
| 쏠림(skew) | 작업·데이터 분포가 불균등해 느린 파티션이 생기는 현상 |
| 디스크 넘김(spill) | 중간 데이터를 메모리에 유지할 수 없어 디스크로 옮기는 것 |
| AQE | 적응형 쿼리 실행. 런타임 통계로 지원되는 실행 계획 선택을 바꾸는 기능 |
