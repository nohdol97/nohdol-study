...
시작 전에 `minikube profile list`로 `infra-study-first`가 이미 존재하지 않는지 확인하고, 원래 kubectl 컨텍스트를 기록한다. 기존 작업에 쓰이는 프로필이면 중단한다.
...
활성 컨텍스트가 `infra-study-first`로 바뀌었는지 확인한다. 정상이라면 노드 하나가 `Ready`로 표시된다.
...
흐름은 `curl → 로컬 port-forward → API/kubelet 터널 → 선택된 Pod 포트`다. Service를 지정하면 kubectl이 Pod를 선택하고 대상 포트를 해석하지만, 패킷이 Service ClusterIP를 통과하지는 않는다. 따라서 이 성공만으로 클러스터 DNS, Service 부하 분산, 일반적인 NetworkPolicy 경로를 검증할 수 없다. 이 경계는 5장의 클러스터 내부 클라이언트로 시험한다. Service가 선택한 백엔드는 다음 명령으로 확인한다.
...
| `kubectl describe pod <name>` | 스케줄링·이미지·probe와 최신 event는 무엇인가? | Pending, pull 실패, 반복 재시작 |
| `kubectl logs <name>` | 컨테이너 프로세스가 무엇을 출력했는가? | 애플리케이션 시작·처리 오류 |
...
다음 단계로 넘어가기 전에 각 `-w` 감시를 Ctrl-C로 종료한다. port-forward도 선택된 Pod가 종료되면 끊긴다. 교체된 Pod가 Ready가 된 뒤 요청을 다시 보내려면 port-forward를 재시작한다.
...
이 실습은 API와 제어 루프를 관찰하기 위한 최소 환경이다. 고가용성, 백업, 업그레이드, 네트워크 정책, 관측과 자원 계획은 [프로덕션 운영과 확장](../../content/kubernetes/10-production-and-extension.md)에서 별도로 다룬다.
...
이미지를 받은 뒤의 예시 출력이다. 자동 생성된 Pod 접미사와 주소는 실행마다 달라진다.
...
롤링 업데이트가 실패해도 기존의 준비된 Pod가 남아 HTTP 요청은 성공할 수 있다. 삭제 실험에서는 Pod UID의 변경과 최종 준비 완료를 기록한다. 대상 Pod가 교체되면 port-forward를 다시 시작하고 HTTP 성공을 재확인한다.
...
[다음 장: API와 오브젝트](../../content/kubernetes/02-api-and-objects.md) · [전체 로드맵](../../content/kubernetes/00-roadmap.md)
