...
|---|---|---|
| operation | 한 remediation 요청과 그 실행·검증 결과를 대표하는 기록 | 재시도가 발생해도 복구 요청 하나의 승인·실행·검증 상태를 추적할 때 쓴다. **구체적인 상황(가상 예시):** 시간 초과된 복구 요청이 이미 실행됐을 수 있다. → 영속 작업 기록을 조회한다. → 효과를 중복시키지 않고 재개·검증·중단을 판단한다. |
| plan | 실행할 target과 예상 diff, precondition을 고정한 변경안 | 상태를 바꾸는 실행을 허용하기 전에 대상·차이·가정을 검토할 때 쓴다. **구체적인 상황(가상 예시):** 용량 추가 변경이 삭제까지 제안한다. → 계획의 정확한 대상과 차이를 검토한다. → 승인 전에 예상하지 못한 삭제를 해결한다. |
| commit | 승인된 plan을 실제 상태 변경으로 전환하는 단계 | 조치 검토와 승인된 실제 효과의 발생 단계를 분리할 때 쓴다. **구체적인 상황(가상 예시):** 복구 제안은 승인됐지만 아직 실행되지 않았다. → 실행 확정 전에 대상·전제조건을 다시 확인한다. → 실제 효과와 후속 검증을 기록한다. |
| reconciliation | 원하는 상태와 실제 상태를 다시 읽어 operation을 수렴시키는 과정 | 응답이 불확실할 때 실제 상태를 확인한 뒤 작업을 반복·수정해 복구한다. **구체적인 상황(가상 예시):** 컨트롤러가 성공한 갱신의 응답을 잃는다. → 목표와 실제 상태를 다시 읽는다. → 아직 필요한 작업만 반복해 상태를 맞춘다. |
| lease | 같은 target을 동시에 바꾸는 executor를 제한하는 시간 있는 소유권 | 실행자들의 임시 소유권을 조정하며 오래된 쓰기를 거부해야 할 때는 fencing도 사용한다. **구체적인 상황(가상 예시):** 워커 둘이 같은 유지 관리 대상을 처리하려 한다. → lease와 필요한 fencing으로 소유권을 조정한다. → 만료와 이전 소유자 쓰기의 거부를 시험한다. |
| verification receipt | action 뒤 어떤 query와 기준으로 성공·실패를 판정했는지 남긴 기록 | 나중에 복구 판정을 감사할 수 있도록 측정값과 판정 기준을 남긴다. **구체적인 상황(가상 예시):** 사고 보고서에 검사 없이 복구됐다고만 적혀 있다. → 질의·기준을 담은 검증 기록을 붙인다. → 남긴 관측으로 판정을 재현한다. |
...
위 digest는 자리표시자다. 실제 계획 해시는 정규화한 대상, 인자, 사전 조건, 범위, 정책·런북 리비전, 만료, 검증 규칙을 포함해야 한다. 효과가 발생하는 경계에서 대상 식별자와 리소스 리비전을 원자적으로 재확인한다. fencing 없는 lease는 지연된 이전 실행자를 막지 못하며 lease 프로토콜 밖의 컨트롤러와도 경쟁할 수 있다. 결과를 모르면 제한된 대사와 에스컬레이션 기한을 두며 무기한 처리 중으로 남겨 두지 않는다.
