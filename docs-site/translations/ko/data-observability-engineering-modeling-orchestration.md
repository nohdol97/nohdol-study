...
|---|---|---|
| 사실 / 차원 | 측정값·이벤트 / 해석에 필요한 설명 맥락 | 측정할 사건과 이를 설명하는 맥락을 나눠 집계·귀속의 의미를 유지한다. |
| 스타 스키마(star schema) | 정의된 키로 사실과 설명 차원을 연결한 구조 | 분석 쿼리에 선언한 행 단위에 맞는 사실·차원 조인 관계를 제공한다. |
| SCD Type 1 / Type 2 | 이전 설명값 덮어쓰기 / 유효 기간 이력 보존 | 보고서가 현재 속성을 쓸지 사건 당시 유효했던 속성을 쓸지 결정한다. |
| 증분 모델(incremental model) | 매번 전체를 만들지 않고 선택한 변경 집합을 처리하는 모델 | 범위가 올바르게 정해진 변경 집합으로 결과를 갱신할 수 있을 때 전체 재계산을 줄인다. |
| 백필(backfill) | 정의한 과거 구간을 다시 처리하는 것 | 무관한 기간을 몰래 바꾸지 않고 누락되거나 정정된 과거 구간을 복구한다. |
| 데이터 구간(data interval) | 예약 계산이 책임지는 시간 범위 | 예약 작업이 책임지는 시간 범위를 명시해 누락 구간·재시도를 확인한다. |
