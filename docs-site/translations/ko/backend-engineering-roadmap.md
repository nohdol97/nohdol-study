...
|---|---|---|
| API 계약 | 호출자가 무엇을 보내고 어떤 결과·오류를 받을지 정한 약속 | 요청·결과·오류의 기대를 명시해 클라이언트와 서버가 호환되게 발전하도록 한다. |
| 불변식 | 요청이 동시에 들어와도 반드시 참이어야 하는 업무 규칙 | 업무 정확성을 모든 동시 실행이 지켜야 할 규칙으로 구체화한다. |
| idempotency | 같은 의도의 요청을 다시 보내도 업무 효과가 한 번으로 수렴하는 성질 | 결과를 확신하기 어려운 요청을 재시도해도 업무 효과가 반복되지 않도록 설계할 때 쓴다. |
| backpressure | 처리할 수 있는 양보다 입력이 많을 때 상류에 속도를 줄이라고 알리는 제어 | 정해진 경계에서 입력을 늦추거나 거부해 과부하가 끝없이 커지는 것을 막는다. |
| outbox | 업무 데이터 변경과 발행할 사건을 한 DB transaction에 함께 기록하는 패턴 | 업무 데이터 커밋과 다른 시스템으로의 발행 사이에서 필요한 이벤트가 유실되는 것을 막는다. |
| 호환 변경 | 구버전과 신버전이 동시에 동작하는 동안에도 소비자를 깨뜨리지 않는 변경 | 순차 배포와 단계적 이전 중에도 기존·신규 소비자가 함께 동작하도록 한다. |
