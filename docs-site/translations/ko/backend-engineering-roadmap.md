...
이 로드맵은 주제 영역을 학습 순서로 연결한다. 지도에 포함됐다고 모든 기법의 완전한 구현이나 운영 측정 결과가 있는 것은 아니다. 각 장의 실습 범위와 완료 기준을 따른다.
...
|---|---|---|
| API 계약 | 호출자가 무엇을 보내고 어떤 결과·오류를 받을지 정한 약속 | 요청·결과·오류의 기대를 명시해 클라이언트와 서버가 호환되게 발전하도록 한다. **구체적인 상황(가상 예시):** API 오류 응답 형식이 바뀌어 클라이언트가 고장 난다. → 응답·오류 계약을 명시한다. → 배포 전에 소비자 호환성 검사를 실행한다. |
| 불변식 | 요청이 동시에 들어와도 반드시 참이어야 하는 업무 규칙 | 업무 정확성을 모든 동시 실행이 지켜야 할 규칙으로 구체화한다. **구체적인 상황(가상 예시):** 동시 구매 두 건이 마지막 재고를 초과 판매할 수 있다. → 재고 불변식과 적용 경계를 정한다. → 함께 커밋돼도 음수 재고가 되지 않는지 시험한다. |
| idempotency | 같은 의도의 요청을 다시 보내도 업무 효과가 한 번으로 수렴하는 성질 | 결과를 확신하기 어려운 요청을 재시도해도 업무 효과가 반복되지 않도록 설계할 때 쓴다. **구체적인 상황(가상 예시):** 서버가 승인한 결제 요청의 응답이 시간 초과된다. → 같은 논리 요청 식별자로 재시도한다. → 결제 효과가 한 번만 기록됐는지 확인한다. |
| backpressure | 처리할 수 있는 양보다 입력이 많을 때 상류에 속도를 줄이라고 알리는 제어 | 정해진 경계에서 입력을 늦추거나 거부해 과부하가 끝없이 커지는 것을 막는다. **구체적인 상황(가상 예시):** 작업 유입이 데이터베이스 커밋보다 빠르다. → 검토한 경계에서 입력을 늦추거나 거부한다. → 대기열 나이와 데이터베이스 회복을 관찰한다. |
| outbox | 업무 데이터 변경과 발행할 사건을 한 DB transaction에 함께 기록하는 패턴 | 업무 데이터 커밋과 다른 시스템으로의 발행 사이에서 필요한 이벤트가 유실되는 것을 막는다. **구체적인 상황(가상 예시):** 주문 커밋 직후 이벤트 발행기가 죽는다. → 주문과 outbox 기록을 한 트랜잭션에 저장한다. → 이후 발행과 소비자 중복 처리를 확인한다. |
| 호환 변경 | 구버전과 신버전이 동시에 동작하는 동안에도 소비자를 깨뜨리지 않는 변경 | 순차 배포와 단계적 이전 중에도 기존·신규 소비자가 함께 동작하도록 한다. **구체적인 상황(가상 예시):** 순차 배포 중 구버전과 신버전이 함께 실행된다. → 양쪽이 필요한 필드를 유지한다. → 이전 지원을 제거하기 전에 혼합 버전 트래픽을 시험한다. |
