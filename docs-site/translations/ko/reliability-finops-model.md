...
- **신뢰성(reliability)**: 필요한 기간 동안 시스템이 기대한 기능을 계속 수행하는 성질이다. **왜 필요한가요 · 언제 쓰나요:** 장애·변경·복구 기간을 포함해 성공적인 동작이 계속 유지되는지 평가한다. **구체적인 상황(가상 예시):** 서비스가 짧은 시험은 통과하지만 배포 때마다 실패한다. → 변경·복구 기간도 신뢰성 측정에 포함한다. → 지속적으로 성공한 결과를 비교한다.
- **SLI/SLO**: 실제 사용자 결과를 재는 방법과 그 측정값의 목표다. **왜 필요한가요 · 언제 쓰나요:** 신뢰성 작업의 우선순위를 정할 때 측정한 사용자 결과를 합의한 목표와 연결한다. **구체적인 상황(가상 예시):** 팀은 가동 시간을 높이는데 고객은 주문 거부를 호소한다. → 주문 결과 SLI와 목표를 정의한다. → 전체 기간의 SLO 달성을 확인한다.
- **failure mode**: 무엇이 어떤 방식으로 실패할 수 있는지를 구체적으로 적은 시나리오다. **왜 필요한가요 · 언제 쓰나요:** 시스템이 고장 나는 구체적인 방식별로 장애 훈련과 통제를 설계한다. **구체적인 상황(가상 예시):** 워커가 출력 커밋 후 응답 전에 멈출 수 있다. → 이 중단을 실패 행렬에 넣는다. → 재생해도 의도한 결과가 유지되는지 확인한다.
- **복구(restore/recovery)**: backup이나 남은 시스템을 사용해 데이터와 서비스를 다시 사용 가능하게 만드는 과정이다. **왜 필요한가요 · 언제 쓰나요:** 보관한 데이터와 대체 구성 요소로 실제 사용 가능한 서비스를 되살릴 수 있는지 확인한다. **구체적인 상황(가상 예시):** 백업은 있지만 애플리케이션과의 호환성을 모른다. → 격리 환경에 복원한다. → 스키마·데이터·권한·전체 사용자 작업을 확인한다.
- **여유 용량(capacity margin)**: 평소 사용량을 넘는 급증이나 일부 장애를 견디기 위해 남겨 둔 처리 능력이다. **왜 필요한가요 · 언제 쓰나요:** 수요 급증이나 복제본 손실 때 남은 용량이 즉시 소진되지 않도록 여유를 둔다. **구체적인 상황(가상 예시):** 복제본 하나가 고장 나면 남은 복제본이 포화된다. → 계획한 장애 상황용 용량 여유를 둔다. → 복제본 하나 없이도 지연이 허용되는지 확인한다.
- **FinOps**: cloud 비용을 기술·업무 소유권과 연결해 측정하고 개선하는 운영 방식이다. **왜 필요한가요 · 언제 쓰나요:** 인프라 지출을 책임 주체와 의미 있는 업무 결과에 연결해 관리할 때 쓴다. **구체적인 상황(가상 예시):** 성공한 주문은 늘지 않았는데 클라우드 비용이 오른다. → 비용을 워크로드 소유자와 유효 결과에 연결한다. → 변경 전후 유효 주문당 비용을 비교한다.
