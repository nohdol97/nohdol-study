...
- **신뢰성(reliability)**: 필요한 기간 동안 시스템이 기대한 기능을 계속 수행하는 성질이다. **왜 필요한가요 · 언제 쓰나요:** 장애·변경·복구 기간을 포함해 성공적인 동작이 계속 유지되는지 평가한다.
- **SLI/SLO**: 실제 사용자 결과를 재는 방법과 그 측정값의 목표다. **왜 필요한가요 · 언제 쓰나요:** 신뢰성 작업의 우선순위를 정할 때 측정한 사용자 결과를 합의한 목표와 연결한다.
- **failure mode**: 무엇이 어떤 방식으로 실패할 수 있는지를 구체적으로 적은 시나리오다. **왜 필요한가요 · 언제 쓰나요:** 시스템이 고장 나는 구체적인 방식별로 장애 훈련과 통제를 설계한다.
- **복구(restore/recovery)**: backup이나 남은 시스템을 사용해 데이터와 서비스를 다시 사용 가능하게 만드는 과정이다. **왜 필요한가요 · 언제 쓰나요:** 보관한 데이터와 대체 구성 요소로 실제 사용 가능한 서비스를 되살릴 수 있는지 확인한다.
- **여유 용량(capacity margin)**: 평소 사용량을 넘는 급증이나 일부 장애를 견디기 위해 남겨 둔 처리 능력이다. **왜 필요한가요 · 언제 쓰나요:** 수요 급증이나 복제본 손실 때 남은 용량이 즉시 소진되지 않도록 여유를 둔다.
- **FinOps**: cloud 비용을 기술·업무 소유권과 연결해 측정하고 개선하는 운영 방식이다. **왜 필요한가요 · 언제 쓰나요:** 인프라 지출을 책임 주체와 의미 있는 업무 결과에 연결해 관리할 때 쓴다.
