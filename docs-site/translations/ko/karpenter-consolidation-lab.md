...
스냅샷을 반복하거나 각 `-w` 감시를 별도 터미널에서 실행하고 Ctrl-C로 멈춘다. 다음 단계가 준비되어도 감시는 자동 종료되지 않는다. 미스케줄 Pod와 스케줄된 뒤 이미지를 기다리는 Pod를 구분한다. EC2 용량을 늘려도 잘못된 이미지는 고쳐지지 않는다.
...
EKS 실행 기록이 아닌 상태 전이 예시다. NodeClaim 이름과 사유 문자열은 설치 버전에 따라 달라진다.
...
Consolidation은 이전 노드, 중단 자격, Pod 이동, 사용자 결과를 기록한다. 예산이나 PDB가 자발적 중단을 지연할 수 있으므로 삭제가 없다고 컨트롤러 실패로 단정하지 않는다. 제공자 회수는 다른 경로를 따르며 대체 용량을 기다리지 않을 수 있다.
