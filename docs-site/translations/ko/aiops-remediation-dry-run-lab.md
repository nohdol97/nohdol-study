...
계획 검토 출력 예시다. 이 워크시트는 실행기나 클러스터 작업을 실행하지 않는다.
...
모든 사전 조건을 채우면 판정이 READY FOR APPROVED EXECUTION이 될 수 있다. 실행과 독립적인 결과 확인 전에는 RECOVERED가 될 수 없다. 같은 작업 키로 반복 요청이 오면 새 롤아웃을 만들지 말고 기존 작업을 확인한다.
...
- 성공·실패 결과를 [incident bundle](../../content/aiops-foundations/02-incident-bundle-contract-lab.md)의 label과 runbook evaluation으로 되돌릴 항목을 정했다.
