...
|---|---|---|
| 유효성(validity) | 값이 정해진 자료형·범위·형식 규칙을 지키는지 뜻한다. | 데이터 공개 전에 허용한 스키마·값의 영역을 위반하는 값을 거부한다. **구체적인 상황(가상 예시):** 업무 규칙상 불가능한 음수 수량이 피드에 들어온다. → 선언한 값 범위 제약으로 검사한다. → 부적합 행을 격리하거나 거부하고 수를 보고한다. |
| 완전성(completeness) | 예상한 레코드와 필수 값이 빠짐없이 있는지 뜻한다. | 도착한 행만 검사해서는 찾을 수 없는 예상 입력의 누락을 발견한다. **구체적인 상황(가상 예시):** 일일 보고서는 정상처럼 보이지만 한 지역 파일이 오지 않았다. → 예상 파티션과 필수 값의 채움을 검사한다. → 도착한 행의 값이 정상이어도 누락 지역을 표시한다. |
| 정확성(accuracy) | 데이터가 실제 사실이나 신뢰할 수 있는 기준과 일치하는지 뜻한다. | 형식상 유효한 데이터도 틀릴 수 있으므로 신뢰할 기준과 실제 내용을 비교한다. **구체적인 상황(가상 예시):** 형식 검사는 통과했는데 배송 주소가 틀리다. → 근거 있는 표본을 신뢰할 수 있는 기준과 대조한다. → 형식 적합성을 정확성으로 간주하지 않고 불일치를 기록한다. |
| 신선도(freshness) | 데이터가 얼마나 최신인지 뜻한다. 어떤 시각과 전달 규칙을 기준으로 볼지 먼저 정해야 한다. | 데이터셋에 정한 시계·소비자 기대를 기준으로 오래된 전달 상태를 찾는다. **구체적인 상황(가상 예시):** 대시보드는 빨리 열리는데 어제 주문만 보여 준다. → 최신 사용 가능 데이터의 소스·처리 시각을 목표와 비교한다. → 조회 서버가 정상이어도 데이터가 오래됐으면 알린다. |
| 계약(contract) | 데이터의 의미·구조·담당자·제공 조건을 합의하고 버전으로 관리한 약속이다. | 생산자·소비자가 의미·변경·서비스 책임에 관해 검토할 수 있는 합의를 갖게 한다. **구체적인 상황(가상 예시):** 생산자가 여러 후속 모델이 쓰는 필드 이름을 바꾼다. → 데이터 계약에 스키마·의미·책임자·호환성 기대를 명시한다. → 배포 전에 소비자 관점에서 변경을 시험한다. |
| 오류 예산(error budget) | SLO를 측정하는 기간에 허용되는 실패량이다. | 남은 실패 허용량을 기준으로 신뢰성 개선과 릴리스 판단의 우선순위를 정한다. **구체적인 상황(가상 예시):** 반복되는 파이프라인 지연이 합의한 최신성 목표를 위협한다. → 정해진 기간의 오류 예산 소모를 계산한다. → 남은 허용량을 복구 작업과 배포 판단의 근거로 사용한다. |
