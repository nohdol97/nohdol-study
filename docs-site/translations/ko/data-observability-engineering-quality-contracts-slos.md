...
| 용어 | 의미 |
|---|---|
| 유효성(validity) | 값이 선언한 타입·범위·형식 규칙을 만족하는지 |
| 완전성(completeness) | 기대한 레코드·필드가 존재하는지 |
| 정확성(accuracy) | 현실의 사실이나 권위 있는 기준과 일치하는지 |
| 신선도(freshness) | 지정한 시계·전달 모델에서 데이터가 얼마나 최신인지 |
| 계약(contract) | 의미·소유권·구조·서비스 기대를 담은 버전 있는 합의 |
| 오류 예산(error budget) | SLO가 정한 기간 안에서 허용하는 실패량 |
