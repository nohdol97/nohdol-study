...
예상 Redis CLI 출력이다. 처음 세 명령은 빠르게 실행한다. 정확한 남은 TTL은 시간에 따라 달라진다.
...
처음 TTL이 9여도 만료는 정상 작동 중이다. -1은 키가 없다는 뜻이 아니라 만료가 없다는 뜻이다. 이 출력은 만료만 보여 준다. 별도 eviction 실습은 `evicted_keys` 증가와 그에 대응하는 캐시 미스를 확인해야 한다. 만료된 키는 eviction 근거가 아니다. TTL 만료 전에 종료한다면 `session:demo`만 삭제한다.
