...
- **packet**: network가 데이터를 나누어 전달하는 작은 단위다. **왜 필요한가요 · 언제 쓰나요:** 요청 실패 시 네트워크 계층의 유실·라우팅·전달을 조사하는 단위로 쓴다. **구체적인 상황(가상 예시):** 연결에서 데이터 재전송이 간헐적으로 발생한다. → 허용된 시험 캡처에서 패킷 근거를 조사한다. → 재전송과 애플리케이션 지연을 비교한다.
