...
|---|---|---|
| 호스트 이름(hostname) | 사람이 기억하기 쉬운 서버 이름. 예: `api.example.com` | 서비스 주소를 이해하기 쉬운 이름으로 유지하면서 실제 주소 기록은 별도로 바꾸도록 한다. **구체적인 상황(가상 예시):** 서비스가 새 서버로 이동한다. → 호스트 이름을 유지하고 해당 주소 기록을 바꾼다. → 영향을 받는 클라이언트에서 이름 해석을 확인한다. |
| IP 주소 | 네트워크에서 컴퓨터나 연결 지점을 찾기 위한 숫자 주소 | 라우팅과 엔드포인트 도달 여부를 확인할 때 네트워크 목적지를 식별한다. **구체적인 상황(가상 예시):** 엔드포인트 하나에는 도달하지만 다른 곳에는 못 간다. → 목적지 IP와 경로를 비교한다. → 요청이 실제로 향하는 주소를 확인한다. |
| DNS | 이름에 대한 레코드를 조회하는 시스템. A/AAAA 레코드로 주소를 조회하며 역방향 조회는 별도 질의다. | 연결 전에 서비스 이름을 해석하고 이름 해석 장애와 전송 장애를 구분한다. **구체적인 상황(가상 예시):** 알고 있는 IP에는 도달하지만 서비스 이름은 해석되지 않는다. → 관련 DNS 기록을 조회한다. → 기록 누락과 연결 장애를 구분한다. |
| 경로(route) | 목적지 IP까지 패킷을 어느 방향으로 보낼지 정한 규칙 | 다음 네트워크 경유지를 정하고 패킷이 목적지에 도달하거나 빗나가는 이유를 찾는다. **구체적인 상황(가상 예시):** 사설 서버가 필요한 목적지에 도달하지 못한다. → 해당 IP에 선택되는 경로를 조사한다. → 다음 경유지가 존재하고 경로를 지원하는지 확인한다. |
| 포트(port) | 한 컴퓨터 안에서 요청을 받을 프로그램을 구분하는 번호 | 트래픽을 원하는 리스너로 보내고 네트워크 도달 여부와 애플리케이션 준비 상태를 구분한다. **구체적인 상황(가상 예시):** 호스트는 응답하지만 8080 포트 요청이 실패한다. → 해당 포트의 리스닝 프로세스를 확인한다. → 바인딩 주소와 연결 결과를 검사한다. |
| 연결(connection) | 두 프로그램이 데이터를 주고받을 수 있도록 만든 통신 상태 | 요청 경로에서 연결 수립·재사용·시간 초과·종료에 드는 비용을 분석할 때 필요하다. **구체적인 상황(가상 예시):** 첫 요청은 느리지만 연결을 재사용한 요청은 빠르다. → 연결 수립과 애플리케이션 처리를 나눠 본다. → 재사용 여부별 시간을 비교한다. |
