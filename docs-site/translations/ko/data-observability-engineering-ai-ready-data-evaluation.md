...
| 용어 | 의미 |
|---|---|
| 임베딩(embedding) | 유사도 기반 검색에 사용하는 수치 표현 |
| 하이브리드 검색(hybrid retrieval) | 어휘 기반과 벡터 기반 검색 신호를 결합하는 것 |
| 재순위화(reranking) | 더 구체적인 관련성 모델로 후보 집합의 순서를 다시 정하는 것 |
| RAG | Retrieval-Augmented Generation, 검색 증강 생성. 검색한 근거를 생성기에 제공하는 방식 |
| 의미 계층(semantic layer) | 지표와 허용 차원·관계를 관리하는 정의 계층 |
| 평가 세트(evaluation set) | 동작 평가에 쓰는 버전 있는 예제·기대 기준 |
