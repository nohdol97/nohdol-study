...
|---|---|---|
| 스냅샷·버전 | 리더가 참조할 수 있도록 식별된 커밋 상태 | 식별된 커밋 테이블 상태를 기준으로 질의·복구 판단을 재현한다. |
| 매니페스트(manifest) | 내용 파일의 집합을 설명하는 Iceberg 메타데이터 | 디렉터리 내용으로 테이블 구성 파일을 추측하지 않고 기록된 메타데이터로 Iceberg 읽기를 계획한다. |
| 카탈로그(catalog) | 구현에 따라 테이블 위치를 찾고 메타데이터를 조정하는 장치 | 기준 테이블 메타데이터를 찾고 카탈로그의 커밋 계약에 따라 갱신을 조정한다. |
| 낙관적 동시성(optimistic concurrency) | 변경을 준비하고 사이에 발생한 변경과 대조해 검증한 뒤 커밋하거나 재시도하는 방식 | 변경 준비는 동시에 하되 커밋 시 충돌하는 변경은 거부하거나 재시도한다. |
| 파일 병합(compaction) | 의도한 행은 유지하면서 파일 배치를 다시 써 단편화를 줄이는 것 | 의도한 테이블 내용을 유지하면서 지나치게 작은 파일들의 부가 비용을 줄인다. |
| 보존(retention) | 버전과 그에 필요한 파일을 사용할 수 있게 유지하는 기간 | 저장 공간이 무제한이라고 가정하지 않고 재생·조사·복구에 필요한 이력을 보존한다. |
