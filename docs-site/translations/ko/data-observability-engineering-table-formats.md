...
|---|---|---|
| 스냅샷·버전 | 식별자가 붙은 커밋 테이블 상태다. 독자는 이 식별자로 특정 시점의 상태를 선택할 수 있다. | 식별된 커밋 테이블 상태를 기준으로 질의·복구 판단을 재현한다. **구체적인 상황(가상 예시):** 오늘 데이터를 수정한 뒤 어제 보고서를 재현해야 한다. → 원래 실행이 사용한 보존된 스냅샷이나 버전을 읽는다. → 식별자와 결과를 보고서 기록과 대조한다. |
| 매니페스트(manifest) | Iceberg에서 데이터 파일이나 삭제 파일의 목록과 관련 정보를 담는 메타데이터 파일이다. | 디렉터리 내용으로 테이블 구성 파일을 추측하지 않고 기록된 메타데이터로 Iceberg 읽기를 계획한다. **구체적인 상황(가상 예시):** Iceberg 쿼리가 스냅샷에 속하는 파일을 찾아야 한다. → 매니페스트 메타데이터와 파일 통계를 따라간다. → 해당 스냅샷과 조건에 맞는 파일이 계획에 선택되는지 확인한다. |
| 카탈로그(catalog) | 엔진이 테이블과 메타데이터를 찾는 데 사용하는 체계다. 변경을 조정하는 역할은 구현마다 다르다. | 기준 테이블 메타데이터를 찾고 카탈로그의 커밋 계약에 따라 갱신을 조정한다. **구체적인 상황(가상 예시):** 두 엔진이 같은 레이크하우스 테이블을 사용해야 한다. → 카탈로그와 네임스페이스를 일관되게 설정한다. → 양쪽에서 같은 테이블과 현재 메타데이터가 조회되는지 확인한다. |
| 낙관적 동시성(optimistic concurrency) | 변경을 먼저 준비하고, 그사이 충돌하는 변경이 있었는지 확인한 뒤 커밋하거나 다시 시도하는 방식이다. | 변경 준비는 동시에 하되 커밋 시 충돌하는 변경은 거부하거나 재시도한다. **구체적인 상황(가상 예시):** 두 작성자가 겹치는 테이블 상태를 동시에 갱신한다. → 커밋 충돌을 감지하고 엔진의 재시도 또는 실패 정책을 적용한다. → 승인된 변경이 조용히 덮어써지지 않는지 확인한다. |
| 파일 병합(compaction) | 작거나 잘게 나뉜 파일을 더 적절한 배치로 다시 쓰는 작업이다. 의도한 데이터 행은 유지한다. | 의도한 테이블 내용을 유지하면서 지나치게 작은 파일들의 부가 비용을 줄인다. **구체적인 상황(가상 예시):** 잦은 적재로 작은 데이터 파일이 수천 개 생겼다. → 대상 파일을 더 적절한 크기로 합친다. → 스캔 부담이 줄고 행 단위 결과는 같은지 확인한다. |
| 보존(retention) | 이전 버전과 그 버전을 읽는 데 필요한 파일을 얼마나 오래 남겨 둘지 정한 기간이다. | 저장 공간이 무제한이라고 가정하지 않고 재생·조사·복구에 필요한 이력을 보존한다. **구체적인 상황(가상 예시):** 분석가가 지난달 버전을 필요로 하는데 저장소 정리를 계획 중이다. → 복구·독자·정책 요구에 맞춰 보존 기간을 정한다. → 삭제 대상 정리 전에 필요한 버전을 읽을 수 있는지 확인한다. |
