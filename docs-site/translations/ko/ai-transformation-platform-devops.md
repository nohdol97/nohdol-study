...
이 CRD는 설치된 API가 아니라 제안한 플랫폼 계약이다. 조직의 스케줄러와 보안 경계에 맞춰야 한다. 큐 승인과 gang scheduling은 학습 작업이 나머지 GPU를 무기한 기다리며 일부만 점유하는 상황을 막아야 한다. 구현은 다르다. Kueue는 할당량, 준비 시간 초과, 관련 스케줄링 기법을 조정하며 승인만으로 모든 워커가 원자적으로 시작했음이 입증되지는 않는다. 준비와 재대기열 동작을 측정하고 선점 비용에 체크포인트 손실도 포함한다.
