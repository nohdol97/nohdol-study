...
|---|---|---|
| 운영체제(OS) | 프로그램과 CPU·메모리·디스크 사이에서 자원을 나누는 기본 소프트웨어 | 여러 프로그램이 공통 인터페이스와 자원 통제를 통해 하드웨어를 함께 쓰도록 한다. **구체적인 상황(가상 예시):** 두 서비스가 한 머신의 메모리를 두고 경쟁한다. → 운영체제의 자원 집계와 프로세스 한도를 조사한다. → 부하 중 어느 서비스가 제한되는지 확인한다. |
| 프로세스(process) | 저장된 프로그램이 실제로 실행되고 있는 한 개의 작업 | 자원 사용·오류·종료 동작이 어느 실행 프로그램에서 생기는지 찾을 때 필요하다. **구체적인 상황(가상 예시):** 서버에 접속은 되지만 웹사이트가 느리다. → 요청을 처리하는 프로세스와 자원 사용량을 찾는다. → 느린 요청과 변화 시점을 비교한다. |
| 서비스(service) | 프로세스를 시작하고 실패하면 다시 시작하도록 운영 이름을 붙인 단위 | 이름이 정해진 워크로드를 일관되게 시작하고 재시작·종료를 관리할 때 쓴다. **구체적인 상황(가상 예시):** 호스트를 재시작하면 API가 사라진다. → 서비스의 시작 절차를 설정·점검한다. → 재부팅 후 실행 상태·리스너·HTTP 요청을 확인한다. |
