...
롤아웃이 실패해도 정상인 이전 복제본이 계속 요청을 처리할 수 있다. 현재 Deployment의 이미지와 준비 상태가 복구되어야 복구 성공으로 판정한다. `logs --previous`는 이전에 종료된 컨테이너가 있어야 하며, 시작조차 못 한 이미지에는 해당 로그가 없다. 관찰을 기록한 뒤 `kubectl delete -f workload.yaml`로 실습 리소스를 삭제한다.
...
[← 클러스터 아키텍처](../../content/kubernetes/03-cluster-architecture.md) · [Service와 네트워킹 →](../../content/kubernetes/05-services-and-networking.md)
