...
| 관계 | 연관 강도 | 활용 방법 | 한계 |
|---|---|---|---|
| 동일 trace·operation ID | 강한 연관 | 같은 실행의 span과 로그 | 샘플링·전송 누락 |
| 동일 배포·변경 ID | 강한 연관 | 변경 집단과 비변경 집단 비교 | 동시 변경이나 공통 의존성 |
...
개별 알림이나 로그 줄이 아니라 사고 유형과 시간으로 데이터셋을 나눈다. 의사결정 시점에 이용 가능한 진단 입력에 사후 분석 결론이나 이후 완화 결과를 넣지 않는다. 미해결과 응답 보류 사고도 명시적으로 평가한다. 응답 범위, 응답한 사례의 정확도, 잘못된 원인 지목 비용, 근거 참조 유효성을 보고한다. 예를 들어 사고 200건 중 100건에 답했고 80건이 맞았다면 응답 범위 50%에서 응답 사례 정확도가 80%인 것이며, 전체 사고 정확도 80%가 아니다. 이 수치는 모델 벤치마크가 아닌 가상 예시다.
