...
- **render**: template에 values를 넣어 Kubernetes가 받을 최종 YAML을 만드는 과정이다. **왜 필요한가요 · 언제 쓰나요:** 설치 전에 최종 매니페스트를 확인해 의도하지 않은 생성 설정을 찾는다.
- **desired state**: Git이나 설정 파일에 “이렇게 되어야 한다”고 선언한 원하는 상태다. **왜 필요한가요 · 언제 쓰나요:** 컨트롤러가 목표와 실제 상태를 비교해 차이를 반복해서 복구하게 할 때 쓴다.
- **live state**: 현재 cluster에 실제로 존재하는 resource의 상태다. **왜 필요한가요 · 언제 쓰나요:** 배포가 목표 상태에 도달했다고 판단하기 전에 실제 객체와 선언한 의도를 비교한다.
- **sync**: desired state와 live state의 차이를 줄이도록 resource를 생성·변경·삭제하는 동작이다. **왜 필요한가요 · 언제 쓰나요:** 조정 과정의 변경 내용을 확인한 뒤 클러스터를 검토한 설정에 맞출 때 쓴다.
- **drift**: 원하는 상태와 실제 상태가 달라진 현상이다. **왜 필요한가요 · 언제 쓰나요:** 검토한 Git 의도와 실제 객체의 차이를 발견하고 어떻게 맞출지 판단한다.
- **CRD**: Kubernetes에 새로운 종류의 resource를 추가하는 정의다. **왜 필요한가요 · 언제 쓰나요:** 도메인에 필요한 객체를 Kubernetes 선언형 API와 컨트롤러 흐름으로 관리할 때 쓴다.
