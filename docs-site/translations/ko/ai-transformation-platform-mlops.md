...
|---|---|---|
| lineage | 산출물이 어떤 입력·실행·부모 artifact에서 나왔는지 잇는 관계 | 결과 재현이나 변경 조사 때 출력을 입력·실행 기록까지 추적한다. **구체적인 상황(가상 예시):** 배포 모델의 결과가 저장된 평가와 다르다. → 두 아티팩트의 데이터·실행 계보를 추적한다. → 바뀐 입력이나 설정을 찾는다. |
| registry | versioned artifact와 alias·metadata를 관리하는 정본 | 검토한 아티팩트 버전을 선택하고 별칭·배포가 가리키는 버전을 기록한다. **구체적인 상황(가상 예시):** 운영 별칭이 예상치 못한 새 모델을 가리킨다. → registry 버전과 승인 기록을 조사한다. → 배포 식별자와 허용된 전환을 확인한다. |
| pipeline | 입력·처리·학습·평가·packaging 단계를 재현 가능한 DAG로 만든 것 | 노트북 실행 순서에 의존하지 않고 여러 단계의 학습 흐름을 재현·검토할 수 있게 한다. **구체적인 상황(가상 예시):** 기억하는 셀 순서로 실행해야만 노트북이 성공한다. → 입력·처리 의존성을 파이프라인으로 만든다. → 깨끗한 실행에서 출력을 재현한다. |
| eval suite | dataset, metric, judge·policy와 threshold가 고정된 평가 묶음 | 버전이 정해진 동일한 예제·승인 기준으로 후보들을 비교한다. **구체적인 상황(가상 예시):** 모델 후보 두 개가 서로 다른 예제로 채점된다. → 평가 묶음과 기준을 고정한다. → 같은 버전에서 두 후보를 비교한다. |
| drift | data·prediction·업무 결과의 분포가 기준선과 달라지는 현상 | 이전 모델 평가나 감시 기준을 더 이상 믿기 어렵게 만드는 분포 변화를 찾을 때 필요하다. **구체적인 상황(가상 예시):** 제품 출시 후 모델이 새로운 고객 집단을 만난다. → 입력·결과 분포를 기준과 비교한다. → 인프라를 원인으로 지목하기 전에 품질을 재평가한다. |
| promotion | candidate가 gate를 통과해 제한된 serving 단계로 이동하는 결정 | 빌드 성공만으로 자격을 갖추지 못한 후보가 서빙에 들어가지 못하게 한다. **구체적인 상황(가상 예시):** 빌드는 성공했지만 안전 평가를 실패했다. → 모든 필수 관문을 승격 조건으로 둔다. → 실패한 후보가 서빙에 들어가지 못하는지 확인한다. |
...
4. 업무 부수 효과를 적용하지 않고 shadow 출력을 비교한다. 차이를 검토할 수 있도록 허용된 평가 기록을 저장한다.
