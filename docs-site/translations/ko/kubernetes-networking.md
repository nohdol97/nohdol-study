...
EndpointSlice는 데이터 평면이 사용하는 API 메타데이터이며 패킷을 전달하는 구성 요소가 아니다. 준비되지 않은 주소가 `ready: false`인 채 남을 수 있으므로, 주소가 전혀 없는 상태와 사용할 수 있는 주소가 없는 상태를 구분한다. readiness 실패는 컨테이너를 재시작하지 않는다. 엔드포인트 전파, 기존 연결, `publishNotReadyAddresses`도 각각 따로 해석해야 한다.
...
curl의 상세 출력으로 클라이언트 이미지에 `nslookup`이 있다고 가정하지 않고 이름 조회, 선택한 주소, 연결, HTTP 응답을 구분한다. 위 진단을 수행하는 동안 이 클라이언트 Pod를 유지한다. 완료 후 임시 클러스터에서 `kubectl delete pod netcheck`와 `kubectl delete -f network.yaml`을 실행한다.
...
클러스터 내부 클라이언트의 예시 출력 일부다. 엔드포인트 주소와 실패 메시지는 네트워크 구현에 따라 달라진다.
...
세 단계 동안 Service ClusterIP가 그대로일 수 있다. Pod 라벨, 준비된 EndpointSlice 주소, 클라이언트 요청 성공이 모두 일치해야 통과다. 셀렉터를 복구한 뒤 시간 제한을 둔 curl을 다시 실행한다. 엔드포인트 복구만으로는 충분하지 않다.
...
[← Pod와 워크로드](../../content/kubernetes/04-pods-and-workloads.md) · [스토리지와 애플리케이션 구성 →](../../content/kubernetes/06-storage-and-configuration.md)
