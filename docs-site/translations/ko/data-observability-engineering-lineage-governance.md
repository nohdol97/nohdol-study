...
|---|---|---|
| job / run / dataset | 잡은 처리할 작업의 정의이고, 실행은 그 잡을 한 번 수행한 것이다. 데이터셋은 읽거나 쓰는 데이터 묶음이다. | 반복되는 변환 정의와 실행 한 번, 해당 실행이 읽고 쓰는 데이터셋을 구분한다. **구체적인 상황(가상 예시):** 일일 변환이 실패해 입력과 출력을 찾아야 한다. → 논리적 잡·이번 실행·관련 데이터셋을 구분한다. → 계보 이벤트의 식별자를 따라 영향을 받는 후속 데이터를 찾는다. |
| facet | OpenLineage의 잡·실행·데이터셋에 붙이는 추가 정보 묶음이다. 스키마 같은 정보를 일정한 형식으로 기록한다. | 모든 도메인에 고정된 기록 형태를 강제하지 않고 구조화된 계보 세부 정보를 덧붙인다. **구체적인 상황(가상 예시):** 계보 이벤트에 기본 식별자 외의 스키마나 실행 정보가 필요하다. → 올바른 개체에 적절한 버전의 facet을 붙인다. → 소비자가 유형과 범위를 일관되게 해석하는지 확인한다. |
| 열 계보(column lineage) | 어느 입력 열이 어느 출력 열을 만드는 데 사용됐는지 나타내는 관계다. | 민감하거나 잘못된 입력 열이 공개된 출력 필드에 미치는 영향을 추적한다. **구체적인 상황(가상 예시):** 민감한 원본 열이 변환을 거쳐 여러 보고서에 들어갈 수 있다. → 수집된 열 계보와 변환 추적 범위를 살핀다. → 계측이 불완전한 부분은 실제 SQL에서 의존성을 확인한다. |
| RBAC / ABAC | RBAC는 역할로 접근 권한을 정한다. ABAC는 요청자·데이터·상황의 속성과 정책을 함께 사용한다. | 정책에 맞는 역할·속성 평가로 누가 데이터에 접근할 수 있는지 표현한다. **구체적인 상황(가상 예시):** 팀 역할별 권한 외에 데이터 분류도 접근 판단에 필요하다. → 역할 기반·속성 기반 조건을 명시적으로 모델링한다. → 대표 신원으로 허용과 거부 조합을 시험한다. |
| 분류(classification) | 데이터를 공개·내부·민감 같은 범주로 나누는 일이다. 분류에 따라 처리 규칙을 적용한다. | 데이터의 민감도에 맞춰 취급·접근·보존 규칙을 적용한다. **구체적인 상황(가상 예시):** 새 데이터셋에 제한적으로 다뤄야 할 연락처가 들어 있다. → 조직이 정의한 범주로 필드를 분류한다. → 후속 접근·보존 규칙이 그 분류를 사용하는지 확인한다. |
| 감사(audit) | 누가 자원에 접근하거나 관리 작업을 했고 무엇을 했는지 남긴 기록이다. | 누가 보호 자원에 접근·변경했는지 조사하고 정책 적용을 사후 검토한다. **구체적인 상황(가상 예시):** 어제 제한 테이블을 누가 조회했는지 설명해야 한다. → 행위자를 식별할 수 있는 접근 감사 이벤트를 적절히 보호해 보존한다. → 기록으로 행위·사용자·대상·시각을 재구성한다. |
...
다음 가상 OpenLineage 이벤트는 완료된 변환을 보여 준다. 완전한 구현은 적절한 수명 주기 이벤트도 보내고 고정 스키마로 검증한다. 예시 생산자 URL은 실제 발생기의 문서화된 신원으로 바꾼다.
