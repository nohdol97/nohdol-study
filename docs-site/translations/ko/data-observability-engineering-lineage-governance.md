...
|---|---|---|
| job / run / dataset | 처리 정의 / 실행 한 번 / 식별된 데이터 집합 | 반복되는 변환 정의와 실행 한 번, 해당 실행이 읽고 쓰는 데이터셋을 구분한다. |
| facet | OpenLineage의 확장 가능한 메타데이터 구조 | 모든 도메인에 고정된 기록 형태를 강제하지 않고 구조화된 계보 세부 정보를 덧붙인다. |
| 열 계보(column lineage) | 입력 열과 출력 열의 관계 | 민감하거나 잘못된 입력 열이 공개된 출력 필드에 미치는 영향을 추적한다. |
| RBAC / ABAC | 역할 기반 접근 / 속성·정책 기반 접근 | 정책에 맞는 역할·속성 평가로 누가 데이터에 접근할 수 있는지 표현한다. |
| 분류(classification) | public·internal·sensitive처럼 취급 규칙에 쓰는 라벨 | 데이터의 민감도에 맞춰 취급·접근·보존 규칙을 적용한다. |
| 감사(audit) | 접근·관리 작업의 보존 기록 | 누가 보호 자원에 접근·변경했는지 조사하고 정책 적용을 사후 검토한다. |
