...
|---|---|---|
| token | tokenizer가 문자열을 모델 vocabulary의 정수 단위로 나눈 값 | 텍스트를 모델의 이산 입력으로 표현하고 문맥 길이·생성 작업량을 측정한다. **구체적인 상황(가상 예시):** 긴 프롬프트가 모델의 문맥 한도를 넘는다. → 맞는 tokenizer로 토큰 수를 센다. → 필요한 근거·지시를 보존하면서 입력을 줄인다. |
| embedding | token ID를 학습 가능한 vector로 바꾼 표현 | 문맥을 반영하는 모델 계산 전에 이산 토큰 ID를 학습 가능한 수치 입력으로 바꾼다. **구체적인 상황(가상 예시):** 언어 모델이 숫자 차이 자체에는 유용한 의미가 없는 토큰 ID를 받는다. → 문맥 계산 전에 학습된 임베딩 표로 벡터에 대응시킨다. → 텐서 형태를 확인하고 토큰 임베딩과 이후 문맥 상태를 구분한다. |
...
LoRA는 기본 가중치를 고정하고 저랭크 가중치 갱신을 학습한다. 어댑터를 재현하려면 정확한 기본 모델, 토크나이저, 프롬프트 템플릿, 학습 설정을 보존해야 한다. 선호 손실 개선만으로 사실 정확성이나 안전한 동작이 입증되지는 않는다.
...
일반적인 attention KV 캐시의 시퀀스당 첫 추정식은 `2 × layers × tokens × KV heads × head dimension × bytes per element`다. 2는 key와 value를 뜻한다. 32개 층, KV head 8개, head 차원 128, 토큰 8,192개, 값당 2바이트이면 시퀀스당 1 GiB이고 같은 시퀀스 32개는 KV만 32 GiB가 필요하다. 모델 가중치, 활성값, 할당기 오버헤드, 런타임 작업 공간은 별도다. GQA는 KV head 수를 바꾸며 잠재 상태를 압축하는 구조는 다른 계산 모델이 필요하다. 벤치마크 전에 이 산술로 불가능한 용량 계획부터 배제한다.
