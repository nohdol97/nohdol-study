...
|---|---|---|
| symptom node | 사용자가 실제로 겪은 오류·지연·기능 실패 | 간접적인 인프라 신호를 따라가기 전에 사용자 피해에 조사 기준을 둔다. **구체적인 상황(가상 예시):** CPU 경보 때문에 사용자의 주문 제출 실패를 놓친다. → 관측한 주문 실패를 그래프의 출발점으로 둔다. → 해당 증상을 설명하는 근거를 따라간다. |
| evidence edge | 두 기록이 같은 요청·배포·자원·시간 창에 속함을 보여 주는 연결 | 시간상 우연한 일치를 입증된 원인으로 오인하지 않도록 근거 관계를 명시한다. **구체적인 상황(가상 예시):** 로그 두 개의 시각은 같지만 서로 다른 요청이다. → 검증한 실행 식별자로 근거를 연결한다. → 뒷받침되지 않는 관계를 제외한다. |
| correlation ID | 서로 다른 신호에서 같은 실행을 찾는 안정적인 식별자 | 타임스탬프만으로 추측하지 않고 실행 하나에 대응하는 로그·트레이스를 찾는다. **구체적인 상황(가상 예시):** 실패 요청이 서비스 세 개를 거친다. → 연결 식별자를 보존하고 검색한다. → 찾은 기록이 해당 실행에 속하는지 확인한다. |
| observation window | incident 분석에 포함할 시작·종료 시간 범위 | 같은 사고 기간의 신호를 비교하고 무관한 이벤트를 제외한다. **구체적인 상황(가상 예시):** 지표와 배포 이벤트의 시간 구간이 달라 모순처럼 보인다. → 관측 구간과 시간대를 맞춘다. → 관련 신호를 다시 비교한다. |
| provenance | evidence가 어느 collector·query·revision에서 왔는지 나타내는 출처 | 운영 주장을 뒷받침하는 수집 방법과 소스 리비전까지 추적할 때 쓴다. **구체적인 상황(가상 예시):** 진단이 수집 출처 없는 지표를 인용한다. → 수집기·질의·리비전을 기록한다. → 질의를 다시 실행해 근거를 확인한다. |
| cardinality | attribute가 가질 수 있는 서로 다른 값의 개수 | 변동이 큰 속성을 텔레메트리에 추가하기 전에 그룹 수·저장량 증가를 예상한다. **구체적인 상황(가상 예시):** 고유 요청 ID 추가로 관측 그룹이 급증한다. → 지표 라벨에 넣기 전에 속성 가짓수를 계산한다. → 변동이 큰 상세 정보는 적절한 진단 신호에 둔다. |
...
- 신호의 기본 역할은 [Observability와 SRE](../../content/observability-sre/01-signals-slo-incident-model.md)에서 이어진다.
- Kubernetes object 상태와 event는 [관측과 트러블슈팅](../../content/kubernetes/09-observability-and-troubleshooting.md)에서 실제 명령으로 확인한다.
- 배포·route change는 [Helm과 GitOps](../../content/helm-gitops/02-render-upgrade-drift-lab.md)와 [트래픽 제어](../../content/traffic-resilience/01-request-budget-and-ownership.md)의 revision으로 연결한다.
- [근거 기반 진단](../../content/aiops-diagnosis/01-detection-correlation-rca.md)은 탐지 점수를 지지·반대 근거로 검증할 수 있는 후보로 바꾼다.
