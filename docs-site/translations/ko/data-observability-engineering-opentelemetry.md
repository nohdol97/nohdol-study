...
| 용어 | 의미 |
|---|---|
| span / trace | 기록된 작업 / trace 컨텍스트로 연결한 관련 작업들 |
| 리소스(resource) | 서비스처럼 텔레메트리 생산 개체를 식별하는 속성 |
| 컨텍스트 전파(context propagation) | 호출·메시지 경계를 넘어 실행 맥락을 전달하는 것 |
| baggage | 요청과 함께 전달할 수 있는 키·값 컨텍스트 |
| OTLP | OpenTelemetry의 텔레메트리 전송 프로토콜 |
| 샘플링(sampling) | 정의한 정책에 따라 보존할 트레이스 정보를 선택하는 것 |
