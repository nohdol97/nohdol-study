...
|---|---|---|
| span / trace | 기록된 작업 / trace 컨텍스트로 연결한 관련 작업들 | 요청 하나의 작업을 여러 연산 경계에 걸쳐 설명하고 지연이 쌓인 곳을 찾는다. **구체적인 상황(가상 예시):** 느린 결제가 API·DB·결제 서비스를 거친다. → 각 작업의 시간 구간을 하나의 분산 추적으로 연결한다. → 계측 누락을 확인하며 지연을 지배하는 경로를 찾는다. |
| 리소스(resource) | 서비스처럼 텔레메트리 생산 개체를 식별하는 속성 | 배포·장애를 비교할 때 관측 데이터를 이를 생성한 서비스·인스턴스에 연결한다. **구체적인 상황(가상 예시):** 스테이징과 운영 지표가 같은 서비스 이름 아래 섞인다. → 서비스와 환경을 식별하는 안정적인 리소스 속성을 붙인다. → 쿼리가 의도한 배포들을 일관되게 구분하는지 확인한다. |
| 컨텍스트 전파(context propagation) | 호출·메시지 경계를 넘어 실행 맥락을 전달하는 것 | HTTP 호출·메시지 처리 경계를 넘어 실행의 연결 관계를 유지한다. **구체적인 상황(가상 예시):** 나가는 HTTP 호출에서 추적이 끊긴다. → 경계 양쪽에서 지원되는 추적 문맥을 주입·추출한다. → 후속 스팬이 의도한 같은 트레이스에 연결되는지 확인한다. |
| baggage | 요청과 함께 전달할 수 있는 키·값 컨텍스트 | 하위 구성 요소에 필요할 때 승인된 작은 문맥 값을 호출 사이에 전달한다. **구체적인 상황(가상 예시):** 여러 서비스에 비민감 라우팅 힌트를 전달해야 한다. → 필요한 경우 허용 목록과 크기 제한을 둔 baggage 필드를 전파한다. → 비밀 정보가 실리거나 크기가 무제한 증가하지 않는지 확인한다. |
| OTLP | OpenTelemetry의 텔레메트리 전송 프로토콜 | 백엔드마다 별도 전송 방식을 만들지 않고 공통 프로토콜로 관측 데이터를 보낸다. **구체적인 상황(가상 예시):** 서비스들이 서로 다른 클라이언트 라이브러리로 관측 데이터를 보낸다. → 호환되는 OTLP 내보내기와 수신기를 설정한다. → 통제된 신호로 전송·속성·백엔드 도착을 확인한다. |
| 샘플링(sampling) | 정의한 정책에 따라 보존할 트레이스 정보를 선택하는 것 | 트레이스의 양·비용을 조절하고 선택 정책으로 잃을 수 있는 근거를 명시한다. **구체적인 상황(가상 예시):** 모든 트레이스를 저장하면 관측 비용 예산을 넘는다. → 조사 목적에 맞는 샘플링 정책을 정한다. → 남은 오류·지연 사례를 측정하고 놓칠 수 있는 범위를 기록한다. |
