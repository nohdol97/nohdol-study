...
|---|---|---|
| span / trace | 스팬은 작업 하나의 기록이다. 트레이스는 관련 스팬들을 연결해 요청이 거친 작업을 따라가게 한다. | 요청 하나의 작업을 여러 연산 경계에 걸쳐 설명하고 지연이 쌓인 곳을 찾는다. **구체적인 상황(가상 예시):** 느린 결제가 API·DB·결제 서비스를 거친다. → 각 작업의 시간 구간을 하나의 분산 추적으로 연결한다. → 계측 누락을 확인하며 지연을 지배하는 경로를 찾는다. |
| 리소스(resource) | 서비스 이름과 배포 환경처럼 관측 데이터를 내보낸 대상을 식별하는 속성이다. | 배포·장애를 비교할 때 관측 데이터를 이를 생성한 서비스·인스턴스에 연결한다. **구체적인 상황(가상 예시):** 스테이징과 운영 지표가 같은 서비스 이름 아래 섞인다. → 서비스와 환경을 식별하는 안정적인 리소스 속성을 붙인다. → 쿼리가 의도한 배포들을 일관되게 구분하는지 확인한다. |
| 컨텍스트 전파(context propagation) | 다음 서비스나 메시지 처리기에 실행 문맥을 전달해 관련 작업을 연결하는 것이다. | HTTP 호출·메시지 처리 경계를 넘어 실행의 연결 관계를 유지한다. **구체적인 상황(가상 예시):** 나가는 HTTP 호출에서 추적이 끊긴다. → 경계 양쪽에서 지원되는 추적 문맥을 주입·추출한다. → 후속 스팬이 의도한 같은 트레이스에 연결되는지 확인한다. |
| baggage | 서비스 경계를 넘는 요청과 함께 전달되는 키·값 형태의 문맥 정보다. | 하위 구성 요소에 필요할 때 승인된 작은 문맥 값을 호출 사이에 전달한다. **구체적인 상황(가상 예시):** 여러 서비스에 비민감 라우팅 힌트를 전달해야 한다. → 필요한 경우 허용 목록과 크기 제한을 둔 baggage 필드를 전파한다. → 비밀 정보가 실리거나 크기가 무제한 증가하지 않는지 확인한다. |
| OTLP | 호환되는 구성 요소 사이에서 관측 데이터를 전송하는 OpenTelemetry 프로토콜이다. | 백엔드마다 별도 전송 방식을 만들지 않고 공통 프로토콜로 관측 데이터를 보낸다. **구체적인 상황(가상 예시):** 서비스들이 서로 다른 클라이언트 라이브러리로 관측 데이터를 보낸다. → 호환되는 OTLP 내보내기와 수신기를 설정한다. → 통제된 신호로 전송·속성·백엔드 도착을 확인한다. |
| 샘플링(sampling) | 정해진 정책에 따라 어떤 추적 정보를 남길지 선택하는 것이다. | 트레이스의 양·비용을 조절하고 선택 정책으로 잃을 수 있는 근거를 명시한다. **구체적인 상황(가상 예시):** 모든 트레이스를 저장하면 관측 비용 예산을 넘는다. → 조사 목적에 맞는 샘플링 정책을 정한다. → 남은 오류·지연 사례를 측정하고 놓칠 수 있는 범위를 기록한다. |
...
앞선 트레이스 파이프라인은 receiver가 OTLP를 받고 메모리 limiter가 이후 처리의 할당 전에 압력을 걸며 batch가 묶고 exporter가 보낸다. 메모리 limiter는 추가 저장소가 아니다. 거부된 신호의 복구는 상위 재시도에 달려 있다. processor는 신호 파이프라인별로 설정하므로 traces용이 지표·로그에도 자동 적용되지 않는다.
