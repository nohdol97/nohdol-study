# OpenTelemetry: 여러 곳에 흩어진 실행 기록 연결하기

데이터 작업이 실패했다. 단서는 스케줄러 로그, 워커 지표, DB 오류 메시지에 흩어져 있다. 어느 기록이 같은 실행에서 나온 것인지 어떻게 알 수 있을까?

OpenTelemetry는 일관된 문맥 정보를 붙여 이런 신호를 수집하고 전송하도록 돕는다. 저장·조회할 시스템과 기록을 연결할 식별자는 별도로 정해야 한다.

## 이 장에서 처음 쓰는 말

| 용어 | 의미 | 왜 필요한가요 · 언제 쓰나요 |
|---|---|---|
| span / trace | 스팬은 작업 하나의 기록이다. 트레이스는 관련 스팬들을 연결해 요청이 거친 작업을 따라가게 한다. | 한 요청이 거친 작업을 연결하고 어느 구간에서 시간이 걸렸는지 찾는다. **구체적인 상황(가상 예시):** 느린 결제가 API·DB·결제 서비스를 거친다. → 각 작업의 시간 구간을 하나의 분산 추적으로 연결한다. → 계측 누락을 확인하며 지연을 지배하는 경로를 찾는다. |
| 리소스(resource) | 서비스 이름과 배포 환경처럼 관측 데이터를 내보낸 대상을 식별하는 속성이다. | 운영·개발 환경이나 서비스 인스턴스가 다른 기록을 구분한다. **구체적인 상황(가상 예시):** 스테이징과 운영 지표가 같은 서비스 이름 아래 섞인다. → 서비스와 환경을 식별하는 안정적인 리소스 속성을 붙인다. → 쿼리가 의도한 배포들을 일관되게 구분하는지 확인한다. |
| 컨텍스트 전파(context propagation) | 다음 서비스나 메시지 처리기에 실행 문맥을 전달해 관련 작업을 연결하는 것이다. | 다른 서비스나 메시지 소비자로 넘어가도 같은 요청의 작업을 이어서 찾는다. **구체적인 상황(가상 예시):** 나가는 HTTP 호출에서 추적이 끊긴다. → 경계 양쪽에서 지원되는 추적 문맥을 주입·추출한다. → 후속 스팬이 의도한 같은 트레이스에 연결되는지 확인한다. |
| baggage | 서비스 경계를 넘는 요청과 함께 전달되는 키·값 형태의 문맥 정보다. | 다음 서비스에도 필요한 작은 문맥 값을 요청과 함께 전달한다. **구체적인 상황(가상 예시):** 여러 서비스에 비민감 라우팅 힌트를 전달해야 한다. → 필요한 경우 허용 목록과 크기 제한을 둔 baggage 필드를 전파한다. → 비밀 정보가 실리거나 크기가 무제한 증가하지 않는지 확인한다. |
| OTLP | 호환되는 구성 요소 사이에서 관측 데이터를 전송하는 OpenTelemetry 프로토콜이다. | 호환되는 도구들이 공통 전송 방식으로 관측 데이터를 주고받게 한다. **구체적인 상황(가상 예시):** 서비스들이 서로 다른 클라이언트 라이브러리로 관측 데이터를 보낸다. → 호환되는 OTLP 내보내기와 수신기를 설정한다. → 통제된 신호로 전송·속성·백엔드 도착을 확인한다. |
| 샘플링(sampling) | 정해진 정책에 따라 어떤 추적 정보를 남길지 선택하는 것이다. | 저장 비용을 조절하면서 어떤 장애 기록을 남기고 놓칠지 관리한다. **구체적인 상황(가상 예시):** 모든 트레이스를 저장하면 관측 비용 예산을 넘는다. → 조사 목적에 맞는 샘플링 정책을 정한다. → 남은 오류·지연 사례를 측정하고 놓칠 수 있는 범위를 기록한다. |
...
지표는 여러 요청의 건수와 지연을 요약하고, 로그는 개별 사건을 기록하며, 트레이스는 작업들이 어떻게 이어졌는지 보여 준다. 리소스 속성은 이 기록을 만든 서비스와 환경을 식별한다. 별도의 저장소를 뜻하지 않는다. 지원되는 도구에서는 프로파일로 어느 코드가 자원을 많이 쓰는지도 볼 수 있다. 실제 사용할 SDK·Collector·백엔드가 필요한 신호와 속성 규약을 지원하는지 확인한다. 언어와 신호 종류마다 지원 수준이 다를 수 있다.
...
배치 하나 읽기, 여러 파티션 변환하기, 새 결과 검사하기, 스냅샷 제공하기처럼 시작과 끝을 정할 수 있는 작업별로 기록한다. 여러 메시지가 한 작업의 입력이라면 부모를 하나만 고르기보다 여러 입력과의 관계를 나타내는 span link를 쓴다. 실행 ID와 이벤트 ID는 계속 늘어나므로 지표 라벨에는 넣지 않는다. 요청마다 별도 시계열이 만들어져 저장량이 크게 늘 수 있기 때문이다.

baggage에 비밀, 프롬프트, 고객 레코드, 접근 토큰을 넣지 않는다. 전파는 신뢰 경계를 넘을 수 있고 baggage는 인가 수단이 아니다. 가능한 한 수집 지점에서 비식별화하고 각 목적지에 실제 도착한 내용을 검증한다.

## 샘플링과 복원력

Head 샘플링은 트레이스가 시작될 때 보관 여부를 정한다. 그 요청이 나중에 실패할지는 아직 모른다. Tail 샘플링은 뒤에 도착한 기록까지 보고 결정할 수 있지만, 기다릴 저장 공간과 같은 트레이스의 스팬들을 한곳으로 모으는 설정이 필요하다. 저장된 트레이스가 없다고 요청이 없었던 것은 아니다. 따라서 SLO의 전체 요청 수는 샘플링한 트레이스 개수가 아니라, 전체 대상을 세는 카운터나 결과 기록에서 가져온다.
...
리소스 속성은 `service.name=orders-transform`, `deployment.environment.name=study`처럼 기록을 만든 서비스와 환경을 나타낸다. 스팬 속성은 데이터셋 이름과 소스 버전처럼 이번 작업을 설명한다. instrumentation scope는 기록을 만든 계측 라이브러리를 식별한다. 실행할 때마다 바뀌는 ID를 리소스에 넣으면 같은 서비스의 기록을 묶어 보기 어려워진다. 서비스 식별 정보와 실행별 정보를 나눠 기록한다.
...
baggage는 다음 서비스에 전달할 키·값 정보이고, 스팬 속성은 현재 작업에 남기는 기록이다. baggage에 넣었다고 자동으로 스팬에 기록되지는 않으며, 스팬 속성에 썼다고 다음 서비스로 전달되지도 않는다. 둘 다 필요하면 허용한 필드만 명시적으로 복사한다. 외부에서 들어온 값의 크기를 제한하고, 비밀이나 임의의 고객 정보가 후속 서비스로 전달되지 않게 한다.
...
이 예제에는 헤더를 정상적으로 읽는 소비자와 빈 컨텍스트로 시작하는 소비자가 있다. 빈 컨텍스트를 명시하는 이유는 같은 프로세스에 남아 있는 생산자의 실행 문맥을 우연히 물려받지 않게 하기 위해서다. 실제로 헤더 전달이 빠졌는데도 같은 트레이스로 보이는 착시를 막는다. 생산자 스팬을 먼저 끝낸 뒤 소비자를 시작해 큐 메시지를 나중에 처리하는 상황도 표현한다.
...
상위 head sampler가 99%를 버리면 하위 tail sampler가 누락 span을 복원할 수 없다. 관측한 오류 전부와 성공 일부를 남기는 정책은 보존 측정 대상을 편향시킨다. SLO는 비샘플 결과 지표나 공개 이력로 세고, 알려진 예제 결과별 생성·보존 트레이스로 샘플링을 평가한다.
...
앞선 설정에서 receiver는 OTLP 데이터를 받는다. `memory_limiter`는 메모리 사용이 커지면 더 받는 양을 제한하고, `batch`는 기록을 묶으며, exporter는 목적지로 보낸다. 메모리 제한기가 거부한 데이터를 대신 저장해 주지는 않는다. 다시 보내려면 앞단의 재시도가 필요하다. 처리기는 신호별 파이프라인에 연결하므로 traces에 설정했다고 지표와 로그에도 적용되는 것은 아니다.
...
영속 큐 저장소는 지원되는 재시작을 넘어 전송 대기 데이터를 보존할 수 있지만 디스크 소진, 복구 불가 목적지, 만료, 영구 오류는 여전히 고려해야 한다. Collector 수신 승인이 백엔드 영속 저장 승인은 아닐 수 있다. 가상 실패 시험에서 생성 ID와 수신·보존 ID를 대조하고 전체 파이프라인 유실은 독립 경로로 텔레메트리 상태를 감시한다.
