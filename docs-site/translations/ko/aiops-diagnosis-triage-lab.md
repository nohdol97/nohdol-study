...
| 준비 항목 | 값 |
|---|---|
| 입력 | 합성 alert 4개, change event 1개, 서비스 dependency 1개 |
| 출력 | 사고 묶음, 근거와 공백, 다음 질의 |
| 정답 상태 | 원인은 아직 미확정 |
| 중단 조건 | 서비스·timestamp·source 중 하나가 해석 불가 |
| cleanup | 운영 변경 없음 |

## 먼저 이해하기

alert 이름이 비슷하다는 이유로 묶으면 같은 template을 쓰는 다른 서비스가 합쳐질 수 있다. timestamp가 가깝다는 이유로 묶으면 정기 batch CPU 상승이 사용자 장애에 섞일 수 있다. 먼저 사용자 영향 범위를 고정하고, trace·dependency·deployment·region 같은 강한 관계를 적용한 뒤, 시간과 문장 유사도는 보조 근거로 쓴다.
...
| 시각 UTC | 종류 | 서비스·resource | 내용 | 사용자 영향 |
|---|---|---|---|---|
| 01:00:30 | change | checkout v18 | image revision 변경 | 아직 모름 |
| 01:03:00 | alert | checkout | success ratio 99.95% → 91% | 확인됨 |
| 01:03:20 | alert | orders-db | 연결 pool pending 급증 | checkout dependency |
| 01:03:40 | alert | checkout proxy | retry overflow 증가 | 요청 경로상 확인 |
| 01:04:00 | alert | search-batch | CPU 92% | 별도 서비스, 영향 없음 |

`checkout → orders-db` dependency는 서비스 catalog와 trace에서 확인됐다고 가정한다. search-batch는 같은 node pool에 있지만 checkout과 trace·dependency가 없고 사용자 영향도 없다. 이 정보만으로 search-batch가 완전히 무관하다고 확정할 수는 없지만 첫 incident 클러스터의 핵심 evidence에서는 제외하고 공유 resource query를 후속으로 남긴다.
...
| orders-db capacity | dependency pending이 symptom 직후 상승 | DB 포화가 먼저였는지 불명 | DB 연결·wait event 원시계열 |
...
여기서 “배포가 먼저”라는 이유만으로 v18을 확정하지 않는다. metric 해상도 때문에 DB pending의 실제 시작이 01:02일 수 있고, 배포와 무관하게 모든 cohort가 실패할 수 있다. 반대로 DB saturation은 v18이 연결을 누수해 만든 중간 원인일 수 있다. root cause는 하나의 node label이 아니라 결함과 방어 실패가 함께 있을 수도 있다.
...
2. `orders-db`의 active·pending 연결과 wait event를 같은 해상도로 조회한다.
...
이 목록은 [incident evidence graph](../../content/aiops-foundations/01-evidence-graph.md)의 edge를 검증한다. query 결과가 나오기 전 LLM에게 원인을 물으면 현재 표를 자연어로 반복할 뿐 새로운 증거를 만들지 못한다.

## 실행 결과 예시

가상 사고에 대한 분석가 출력 예시이며 모델 벤치마크나 확정된 근본 원인이 아니다.
...
의존 관계가 없는 무관한 서비스의 알림은 시각이 같아도 분리한다. 묶은 각 알림과 후보를 제공된 근거까지 추적할 수 있고 반증이 유지되며 뒷받침 없는 주장을 보류해야 통과다. 묶음 수가 줄었다는 이유만으로 진단 품질이 높아지는 것은 아니다.
...
- 이 클러스터 결과를 [자동 복구 dry-run](../../content/aiops-remediation/02-remediation-dry-run-lab.md)에 넘길 최소 조건은 무엇인가?
