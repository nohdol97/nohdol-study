...
|---|---|---|
| representation | 원본 입력을 모델이 계산할 수 있는 숫자 구조로 바꾼 것 | 원시 입력을 모델이 유용한 차이를 학습할 수 있는 특징으로 바꿀 때 쓴다. |
| objective | 모델이 줄이도록 학습하는 loss와 업무상 최적화 목표 | 학습 목표를 명시하고 실제 업무 결과와 맞는지 확인할 때 쓴다. |
| inference | 학습한 모델로 새 입력의 결과를 계산하는 과정 | 측정한 지연·용량·품질 제약 안에서 학습된 모델을 새 입력에 적용한다. |
| baseline | 복잡한 모델이 실제로 이겨야 하는 단순 비교 기준 | 같은 평가 조건에서 복잡한 모델이 간단한 기준보다 개선됐는지 판단한다. |
| retrieval | 답을 만들기 전에 외부 자료에서 후보 근거를 찾는 단계 | 답에 모델 파라미터 밖의 정보가 필요할 때 관련 외부 근거를 찾는다. |
| evaluation | 정답·품질·비용·안전·지연을 고정한 조건에서 측정하는 일 | 후보를 다음 단계로 올리기 전에 명시한 품질·운영 요구를 만족하는지 판단한다. |
