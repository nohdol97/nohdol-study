...
|---|---|---|
| representation | 원본 입력을 모델이 계산할 수 있는 숫자 구조로 바꾼 것 | 원시 입력을 모델이 유용한 차이를 학습할 수 있는 특징으로 바꿀 때 쓴다. **구체적인 상황(가상 예시):** 모델이 직접 계산할 수 없는 고객 원문을 받는다. → 정해진 표현 처리 과정으로 변환한다. → 모양·전처리·학습과 서빙의 일관성을 확인한다. |
| objective | 모델이 줄이도록 학습하는 loss와 업무상 최적화 목표 | 학습 목표를 명시하고 실제 업무 결과와 맞는지 확인할 때 쓴다. **구체적인 상황(가상 예시):** 추천 모델의 클릭은 늘었지만 합의한 업무 결과는 나빠졌다. → 학습 목표와 평가 기준을 검토한다. → 실제 목표 결과를 비교한다. |
| inference | 학습한 모델로 새 입력의 결과를 계산하는 과정 | 측정한 지연·용량·품질 제약 안에서 학습된 모델을 새 입력에 적용한다. **구체적인 상황(가상 예시):** 모델이 오프라인 평가는 통과하지만 서빙 지연 요구를 놓친다. → 대상 런타임·부하에서 추론을 측정한다. → 품질·지연·용량을 함께 확인한다. |
| baseline | 복잡한 모델이 실제로 이겨야 하는 단순 비교 기준 | 같은 평가 조건에서 복잡한 모델이 간단한 기준보다 개선됐는지 판단한다. **구체적인 상황(가상 예시):** 복잡한 분류기가 좋아 보이지만 비교 기준이 없다. → 같은 데이터 분할과 지표로 간단한 기준 모델을 평가한다. → 추가 복잡성이 의미 있는 개선을 만드는지 확인한다. |
| retrieval | 답을 만들기 전에 외부 자료에서 후보 근거를 찾는 단계 | 답에 모델 파라미터 밖의 정보가 필요할 때 관련 외부 근거를 찾는다. **구체적인 상황(가상 예시):** 답변에 현재 사내 반품 정책이 필요하다. → 접근이 허용된 정책 구간을 검색한다. → 답변 생성 전에 버전과 관련성을 확인한다. |
| evaluation | 정답·품질·비용·안전·지연을 고정한 조건에서 측정하는 일 | 후보를 다음 단계로 올리기 전에 명시한 품질·운영 요구를 만족하는지 판단한다. **구체적인 상황(가상 예시):** 후보가 데모 하나는 개선하지만 다른 과제는 악화시킬 수 있다. → 고정된 평가 집합과 운영 검사를 실행한다. → 평균 점수와 실패 사례를 함께 비교한다. |
