...
apply 명령은 API 오브젝트를 제출한다. API 서버가 원하는 상태를 저장한 뒤 컨트롤러, 스케줄러, kubelet이 각자의 비동기 작업을 수행해야 컨테이너가 실행된다.
...
명시한 JSONPath 조회 두 개의 예상값과 대표적인 소유자 조회 출력이다. 오브젝트 이름에는 자동 생성 접미사가 포함된다.
...
같은 ConfigMap 이름도 네임스페이스가 다르면 다른 값으로 해석된다. 네임스페이스 옵션을 빠뜨리면 NotFound나 다른 값이 나올 수 있으며, 이것만으로 데이터가 없다고 판단할 수 없다. 소유 관계는 Deployment → ReplicaSet → Pod여야 한다. apply 성공만으로 복제본 두 개가 준비됐다고 볼 수는 없다.
