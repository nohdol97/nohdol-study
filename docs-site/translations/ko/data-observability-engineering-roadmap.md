...
| 처음 만나는 말 | 학습용 쉬운 뜻 |
|---|---|
| 데이터 제품(data product) | 사용자, 의미, 담당자, 전달 약속이 정의된 데이터셋 |
| 파이프라인(pipeline) | 데이터를 수집·변환·검증·공개하는 단계들 |
| 관측성(observability) | 시스템 내부 동작을 설명하는 데 도움이 되는 근거 |
| 데이터 신뢰성(data reliability) | 합의한 시간과 복구 범위 안에서 필요한 정확성·완전성을 갖춘 데이터를 전달하는 것 |
| 계보(lineage) | 입력, 처리 실행, 출력 사이의 관계 기록 |
| SLI / SLO | 측정한 결과 / 그 결과에 대해 합의한 목표 |
