...
- **끝난 상태**: 임시 rule과 test 트래픽을 제거하고 원래 metric 추세로 돌아왔는지 확인한다.
...
`http_server_requests_total` 같은 counter는 프로세스가 시작된 뒤 누적된다. 따라서 현재 값끼리 나누기보다 일정 window의 `rate`를 사용한다. histogram bucket은 각 latency 경계 이하의 누적 요청 수이며 `histogram_quantile`이 여러 bucket을 이용해 percentile을 추정한다. 개별 요청의 정확한 시간을 보여 주는 trace와 역할이 다르다.

| 관찰 | 알 수 있는 것 | 주의할 점 |
|---|---|---|
| 5분 error ratio | 최근 트래픽에서 실패 비중 | 트래픽이 적으면 작은 수에도 크게 흔들림 |
...
트래픽이 0일 때 분모가 0이 되는 상황, retry가 요청 수를 늘리는 상황과 ingress/client 중 어느 지점에서 측정하는지를 기록한다.
...
| T1 | DB span latency 증가 | 연결 saturation | pool metric 확인 | active=max |
...
availability 식의 값이 `0.98`이면 선택한 5분 window와 label 범위에서 약 98%가 good으로 분류됐다는 뜻이다. 어떤 request를 valid 또는 good에서 제외했는지에 따라 의미가 달라진다. health check나 클라이언트 cancel을 무심코 분모에서 빼면 실제 사용자 실패를 숨길 수 있다.

p95가 상승한 시각과 DB span 증가가 겹치면 dependency 병목 가설이 강해지지만 아직 인과관계가 확정된 것은 아니다. 같은 trace의 parent-child 시간, 연결 pool, DB wait와 변경 시점을 함께 본다. 완화 후에는 단일 성공 요청뿐 아니라 burn rate, tail latency와 backlog가 정상 범위로 돌아오는지 확인한다.
