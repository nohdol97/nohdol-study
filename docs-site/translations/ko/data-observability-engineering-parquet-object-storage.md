...
| 용어 | 의미 |
|---|---|
| 행 그룹(row group) | 여러 행을 수평으로 묶고 각 열을 별도 청크로 저장한 단위 |
| 열 청크(column chunk) | 한 행 그룹 안의 특정 열 데이터 |
| 페이지(page) | 열 청크 내부의 더 작은 인코딩 단위 |
| 열 제거(column pruning) | 쿼리에 필요한 열만 읽는 것 |
| 조건 밀어 넣기(predicate pushdown) | 읽는 계층에 필터를 전달해 작업을 피하거나 줄이는 것 |
| 오브젝트 키(object key) | 저장 서비스에서 오브젝트 하나를 식별하는 이름 |
