...
이 검사들은 서로 다른 경계를 확인한다. API가 준비됐어도 워크로드에는 준비된 복제본이 없을 수 있다. 리소스가 아닌 readiness URL에 접근 권한이 없는 사용자는 Forbidden을 받을 수 있다. 이는 인가 결과이며 API 장애 진단이 아니다. 실습 후 직접 만든 임시 `object.yaml` 리소스만 삭제한다.
