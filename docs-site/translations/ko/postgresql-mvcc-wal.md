...
- **commit**: transaction의 변경을 최종 성공으로 확정하는 동작이다. **왜 필요한가요 · 언제 쓰나요:** 트랜잭션의 승인된 변경이 설정한 성공·영속성 경계에 도달했음을 확정할 때 쓴다.
- **rollback**: transaction에서 수행한 변경을 취소하고 시작 전 상태로 돌아가는 동작이다. **왜 필요한가요 · 언제 쓰나요:** 오류나 업무 규칙 검사 실패 뒤 완료되지 않은 트랜잭션의 변경을 취소할 때 쓴다.
- **MVCC**: 동시에 실행되는 transaction이 서로에게 어떤 버전의 row를 보여 줄지 관리하는 방식이다. **왜 필요한가요 · 언제 쓰나요:** 동시 트랜잭션의 읽기를 모두 배타적 쓰기 잠금으로 처리하지 않고 버전 가시성을 정한다.
- **WAL**: data file을 바꾸기 전에 변경 내용을 순서대로 남기는 로그다. 장애 복구와 복제의 기반이 된다. **왜 필요한가요 · 언제 쓰나요:** 영속적인 데이터베이스 진행 상태를 복구·복제하는 데 필요한 순서 있는 변경 기록을 남긴다.
- **VACUUM**: 더 이상 어떤 transaction에도 필요하지 않은 예전 row 공간을 다시 사용할 수 있게 정리하는 작업이다. **왜 필요한가요 · 언제 쓰나요:** 더 이상 필요 없는 행 공간을 재사용하고 PostgreSQL의 트랜잭션 나이 유지 관리 요구를 처리한다.
- **query plan**: PostgreSQL이 SQL을 실행하기 위해 선택한 table 접근 순서와 방법이다. **왜 필요한가요 · 언제 쓰나요:** 인덱스나 SQL을 바꾸기 전에 비싼 스캔·조인·추정 오차를 찾는 데 쓴다.
...
그림은 일반적인 WAL 기록 테이블, `fsync=on`, 로컬 WAL 기록 완료를 기다리는 커밋 모드를 가정한다. `synchronous_commit=off`에서는 데이터베이스 일관성이 유지되더라도 승인된 트랜잭션이 장애 후 유실될 수 있다. 로컬 내구성만으로 대기 서버의 커밋 수신·적용도 입증되지 않는다. 장애 조치 RPO를 정하기 전에 동기 복제 설정을 확인한다.

## VACUUM의 책임

UPDATE와 DELETE는 기존 스냅샷에서 보일 수 있는 이전 행 버전을 남긴다. VACUUM은 더 이상 필요하지 않은 버전의 공간만 회수할 수 있다. 가시성 맵도 관리하고 트랜잭션 ID 순환 문제를 방지하는 데 도움을 준다. 일반 VACUUM과 테이블을 다시 쓰며 더 강한 잠금을 요구하는 `VACUUM FULL`을 동일하게 취급하지 않는다.
