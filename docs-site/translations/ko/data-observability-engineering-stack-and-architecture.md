...
|---|---|---|
| 저장 형식(storage format) | 파일 안에 데이터를 어떤 형태로 저장할지 정한 규칙이다. Parquet가 한 예다. | 분석 접근·압축·상호운용 요구에 맞는 파일 표현을 선택한다. **구체적인 상황(가상 예시):** 질의에 넓은 데이터셋의 필드 두 개만 필요하다. → 해당 접근 방식으로 파일 형식을 비교한다. → 읽은 바이트·해독 작업·출력 정확성을 측정한다. |
| 테이블 형식(table format) | 커밋된 테이블 버전에 어떤 파일이 속하는지 정하는 규칙과 메타데이터다. | 읽기·쓰기가 동시에 일어날 때 어떤 파일들이 승인된 테이블 상태를 이루는지 정한다. **구체적인 상황(가상 예시):** 쓰는 쪽은 파일을 추가하고 읽는 쪽은 일관된 테이블이 필요하다. → 테이블 형식의 커밋 상태 절차를 쓴다. → 독자가 디렉터리 추측이 아닌 유효한 한 버전을 읽는지 확인한다. |
| 계산 엔진(compute engine) | 쿼리나 변환을 실행해 데이터에서 결과를 계산하는 소프트웨어다. | 필요한 변환을 실행하고 선택한 계획이 발생시키는 작업량을 측정한다. **구체적인 상황(가상 예시):** 변환이 데이터를 읽지만 시간 대부분을 재분배에 쓴다. → 계산 엔진의 계획을 조사한다. → 같은 결과 검사로 수정한 계획을 비교한다. |
| 제어 평면(control plane) | 무엇을 언제 실행하고 어떤 권한을 사용할지 정하는 플랫폼의 관리 부분이다. | 작업을 지시하는 정의·정책과 데이터를 실제 이동시키는 실행을 분리한다. **구체적인 상황(가상 예시):** 작업 정의는 있지만 일정이나 권한이 틀렸다. → 제어 계층의 메타데이터·정책을 조사한다. → 의도한 실행이 허용되고 예약됐는지 확인한다. |
| 데이터 평면(data plane) | 데이터를 실제로 읽고 옮기고 변환하는 워커와 저장소다. | 데이터가 실제 처리되는 곳의 처리량·장애·접근 경계를 찾는다. **구체적인 상황(가상 예시):** 예약은 성공하지만 실제 데이터 전송이 멈춘다. → 읽기·워커·저장소의 실행 계층을 조사한다. → 측정된 처리량·접근 병목을 찾는다. |
