...
|---|---|---|
| Gateway | 외부 또는 다른 서비스에서 들어온 요청을 받는 공용 진입점 | 리스너·인증서·경로 연결처럼 트래픽 진입점에 필요한 정책을 모을 때 쓴다. |
| Route | 어떤 조건의 요청을 어느 backend로 보낼지 적은 규칙 | 조건에 맞는 애플리케이션 요청을 명시적인 규칙에 따라 원하는 백엔드로 보낸다. |
| upstream | proxy가 대신 요청을 보내는 뒤쪽 서비스 | 전달 장애를 진단할 때 프록시가 다음으로 의존하는 서비스를 식별한다. |
| deadline | 최초 요청부터 최종 응답까지 허용한 전체 시간 | 중첩 작업이 시간을 무제한 소비하지 않도록 사용자의 전체 대기 시간을 제한한다. |
| retry budget | 정상 요청량에 비례해 허용하는 추가 시도 상한 | 재시도가 정상적인 첫 시도에 필요한 용량까지 소모하지 않도록 제한한다. |
| circuit breaker | 연결·대기·요청·재시도가 상한을 넘으면 빠르게 거부하는 경계 | 연결·요청 한도를 넘긴 작업이 과부하를 악화시키기 전에 일찍 거부한다. |
| outlier detection | 반복 실패하는 backend를 일정 시간 healthy 집합에서 제외하는 기능 | 정상 대안이 있을 때 반복 실패하는 인스턴스로 가는 트래픽을 줄인다. |
| blast radius | 한 변경이나 실패가 영향을 미칠 수 있는 범위 | 실험·조치의 범위를 제한해 실수 하나가 영향을 주는 사용자·자원 수를 줄인다. |
