...
|---|---|---|
| concurrency | 같은 시간에 시작되어 아직 끝나지 않은 작업 수 | 진행 중인 작업 수를 제한해 자원 사용과 대기열 증가를 통제할 때 쓴다. **구체적인 상황(가상 예시):** 동시 요청을 늘리자 완료보다 대기가 증가한다. → 진행 중인 요청 수를 제한한다. → 유효 처리량·지연·대기열 증가를 비교한다. |
| throughput | 단위 시간에 성공적으로 끝난 업무 수 | 용량이나 성능 변경을 비교할 때 실제로 완료한 유용한 작업을 측정한다. **구체적인 상황(가상 예시):** 새 워커 설정에서 초당 시도 수가 늘었다. → 성공적으로 완료한 업무를 센다. → 같은 부하·정확성 검사 아래 처리량을 비교한다. |
| queue | 실행 자리가 날 때까지 기다리는 작업의 모음 | 유입과 처리 속도의 일시적인 차이를 흡수하되 적체 한도 정책을 함께 둔다. **구체적인 상황(가상 예시):** 할인 행사 동안 작업 유입이 처리보다 잠시 빠르다. → 대기열 용량 정책 안에서 보관한다. → 가장 오래된 항목의 나이와 적체 해소를 추적한다. |
| backpressure | 하류가 포화될 때 상류 입력을 늦추거나 거절하는 제어 | 정해진 경계에서 입력을 늦추거나 거부해 과부하가 끝없이 커지는 것을 막는다. **구체적인 상황(가상 예시):** 작업 유입이 데이터베이스 커밋보다 빠르다. → 검토한 경계에서 입력을 늦추거나 거부한다. → 대기열 나이와 데이터베이스 회복을 관찰한다. |
| saturation | 자원이 더 많은 일을 받아도 유용한 처리량이 늘지 않는 상태 | 작업을 더 넣어도 처리량이 늘지 않고 지연·실패만 증가하는 지점을 찾는다. **구체적인 상황(가상 예시):** 트래픽을 두 배로 늘려도 완료는 비슷하고 지연만 증가한다. → 포화 자원이나 하위 단계를 찾는다. → 입력 감소로 안정성이 좋아지는지 시험한다. |
| GC pause | runtime이 회수할 메모리를 찾는 동안 application 진행이 영향을 받는 시간 | 평균 CPU·처리량 측정에 가려질 수 있는 지연 급증을 설명할 때 필요하다. **구체적인 상황(가상 예시):** 주기적 응답 지연이 가비지 수집과 겹친다. → 런타임 중단 기록과 요청 시간을 비교한다. → 힙·동시성 설정을 바꾸기 전에 관계를 확인한다. |
