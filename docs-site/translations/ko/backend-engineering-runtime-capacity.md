...
|---|---|---|
| concurrency | 같은 시간에 시작되어 아직 끝나지 않은 작업 수 | 진행 중인 작업 수를 제한해 자원 사용과 대기열 증가를 통제할 때 쓴다. |
| throughput | 단위 시간에 성공적으로 끝난 업무 수 | 용량이나 성능 변경을 비교할 때 실제로 완료한 유용한 작업을 측정한다. |
| queue | 실행 자리가 날 때까지 기다리는 작업의 모음 | 유입과 처리 속도의 일시적인 차이를 흡수하되 적체 한도 정책을 함께 둔다. |
| backpressure | 하류가 포화될 때 상류 입력을 늦추거나 거절하는 제어 | 정해진 경계에서 입력을 늦추거나 거부해 과부하가 끝없이 커지는 것을 막는다. |
| saturation | 자원이 더 많은 일을 받아도 유용한 처리량이 늘지 않는 상태 | 작업을 더 넣어도 처리량이 늘지 않고 지연·실패만 증가하는 지점을 찾는다. |
| GC pause | runtime이 회수할 메모리를 찾는 동안 application 진행이 영향을 받는 시간 | 평균 CPU·처리량 측정에 가려질 수 있는 지연 급증을 설명할 때 필요하다. |
