...
| topology | service·dependency·deployment·resource가 어떻게 연결되는지 나타낸 관계 | 사고 중 의존성을 따라 가능한 원인과 영향을 받는 사용자의 범위를 좁힌다. **구체적인 상황(가상 예시):** 결제 오류가 공유 의존성에서 시작됐을 수 있다. → 관련 topology 연결을 따라간다. → 영향·비영향 서비스가 설명을 뒷받침하는지 확인한다. |
...
1. [운영 신호를 incident evidence graph로 연결하기](../../content/aiops-foundations/01-evidence-graph.md)에서 symptom → service → deployment → resource → trace 관계와 시간 창을 만든다.
