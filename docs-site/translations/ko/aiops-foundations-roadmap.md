...
먼저 [Observability와 SRE](../../content/observability-sre/00-roadmap.md)에서 metric·log·trace·SLO의 역할을 익힌다. [AI Specialist](../../content/ai-specialist-core/00-roadmap.md)의 model·retrieval 결과와 [AI Transformation](../../content/ai-transformation-platform/00-roadmap.md)의 bundle·deployment·tool operation ID도 이 단계에서 운영 신호와 만난다. 여기서는 여기에 Kubernetes event, deployment·configuration change, 서비스 owner와 topology를 더해 `incident bundle`을 만든다. 이 bundle이 있어야 [이상 탐지와 장애 진단](../../content/aiops-diagnosis/00-roadmap.md)이 매번 다른 데이터를 임의로 읽지 않고 같은 사건을 재현할 수 있다.

| 처음 만나는 말 | 학습용 쉬운 뜻 | 왜 필요한가요 · 언제 쓰나요 |
|---|---|---|
| AIOps | 운영 데이터를 분석해 탐지·진단·조치 결정을 보조하는 기술과 운영 절차 | 수동 분류만으로 감당하기 어려울 때 대응자의 운영 근거 정리와 판단을 돕는다. **구체적인 상황(가상 예시):** 대응자가 관련 운영 신호 수백 개를 받는다. → AIOps 절차로 근거와 후보를 정리한다. → 조치 전에 진단을 검증 가능한 상태로 유지한다. |
| telemetry | 시스템이 밖으로 내보내는 metric·log·trace 같은 관측 데이터 | 문제 해결·서비스 측정·사고 분석에 사용할 관측 근거를 제공한다. **구체적인 상황(가상 예시):** 서비스는 정상인데 감시 데이터가 끊긴다. → 관측 데이터 생성과 전송을 따로 조사한다. → 누락을 성공이 아니라 관측 불명으로 표시한다. |
| semantic convention | 서로 다른 시스템이 같은 의미를 같은 이름과 단위로 기록하기 위한 약속 | 이름·단위·의미가 몰래 섞이지 않도록 여러 서비스의 신호를 연결·비교한다. **구체적인 상황(가상 예시):** 두 서비스가 같은 시간을 다른 단위로 보고한다. → 정해진 의미 규약을 적용한다. → 서비스 간 질의의 이름·단위 호환성을 확인한다. |
| topology | 서비스·dependency·deployment·resource가 어떻게 연결되는지 나타낸 관계 | 사고 중 의존성을 따라 가능한 원인과 영향을 받는 사용자의 범위를 좁힌다. **구체적인 상황(가상 예시):** 결제 오류가 공유 의존성에서 시작됐을 수 있다. → 관련 topology 연결을 따라간다. → 영향·비영향 서비스가 설명을 뒷받침하는지 확인한다. |
| change event | 배포·설정·feature flag처럼 시스템 동작을 바꾼 사건 | 최근 배포·설정 변경이 관측한 증상을 설명할 수 있는지 검토한다. **구체적인 상황(가상 예시):** 설정 배포 직후 오류가 시작된다. → 변경 이벤트와 증상 시점을 맞춘다. → 변경이 원인이라고 확정하기 전에 다른 설명을 시험한다. |
| incident bundle | 한 장애의 시간 범위, 영향, 신호, 변경, 후보와 판단을 묶은 재현 가능한 기록 | 조사자들에게 범위가 정해진 같은 근거 집합을 제공해 진단을 재현·검토한다. **구체적인 상황(가상 예시):** 조사자 둘이 다른 시간 구간을 보고 다른 결론을 낸다. → 버전이 정해진 사고 묶음을 공유한다. → 같은 근거로 질의를 재현한다. |
| ground truth | 사건이 끝난 뒤 사람과 증거로 확정한 원인·영향·조치 label | 모델 간 동의 대신 사후 확인한 사고 결과를 기준으로 진단 예측을 평가한다. **구체적인 상황(가상 예시):** 자동 진단을 다른 모델의 추측과 비교해 채점한다. → 근거를 검토한 사고 결과를 기준으로 삼는다. → 미해결 사례는 표시하고 정답을 만들지 않는다. |
...
1. [운영 신호를 incident evidence graph로 연결하기](../../content/aiops-foundations/01-evidence-graph.md)에서 symptom → 서비스 → deployment → resource → trace 관계와 시간 창을 만든다.
2. [Incident bundle 데이터 계약 검토 실습](../../content/aiops-foundations/02-incident-bundle-contract-lab.md)에서 작은 JSON record의 누락을 찾아 같은 사건을 재현할 수 있는지 확인한다.
3. [Alert 묶기와 진단 근거 선택](../../content/aiops-diagnosis/02-alert-correlation-triage-lab.md)으로 bundle을 넘겨 여러 alert를 incident 하나로 묶는다.
4. [승인된 자동 복구](../../content/aiops-remediation/00-roadmap.md)에서 진단 결과가 어떤 권한과 검증을 거쳐 조치가 되는지 연결한다.
...
| 어디서 깨졌나 | hostname 문자열 | 안정적인 서비스·resource·deployment ID |
...
- 서비스 name, deployment revision, resource ID 중 어느 식별자가 어떤 관계를 고정하는지 말할 수 있다.
