...
|---|---|---|
| control plane | 어떤 listener와 route, 정책이 존재해야 하는지 선언·검증하는 층 | 요청 실행과 별도로 트래픽 의도를 검토하고 설정이 수용됐는지 확인한다. |
| data plane | 실제 요청을 받아 backend로 전달하고 timeout·retry·limit를 집행하는 층 | 설정한 라우팅·시간 제한·재시도 정책이 실제 요청에 적용되는 지점을 관찰한다. |
| route attachment | Route와 Gateway가 서로의 조건을 만족해 실제 연결되는 과정 | 백엔드 네트워크를 원인으로 지목하기 전에 선언한 경로가 Gateway에 연결될 조건을 갖췄는지 확인한다. |
| per-try timeout | 한 번의 upstream 시도에 허용하는 시간 | 느린 시도 하나를 제때 중단해 전체 요청의 시간 예산을 남긴다. |
| outer deadline | 최초 요청 수신부터 최종 응답까지의 전체 상한 | 중첩된 모든 시도와 대기가 호출자의 전체 허용 시간 안에 들어가게 한다. |
| retry storm | 실패 중 추가 시도가 부하를 키워 실패를 더 만드는 현상 | 장애 중 재시도가 부하를 증폭시키는 상황을 알아보고 위험한 재시도를 제한·중지한다. |
