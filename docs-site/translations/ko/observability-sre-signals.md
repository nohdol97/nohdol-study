...
- **telemetry**: 시스템이 자신의 상태를 밖에서 관찰할 수 있도록 내보내는 metric·log·trace 데이터다. **왜 필요한가요 · 언제 쓰나요:** 문제 해결·서비스 측정·사고 분석에 사용할 관측 근거를 제공한다.
- **latency**: 요청을 보낸 뒤 응답을 받을 때까지 걸린 시간이다. **왜 필요한가요 · 언제 쓰나요:** 사용자 대기 시간을 측정하고 요청 시간 예산을 조정하기 전에 느린 단계를 찾는다.
- **error rate**: 전체 요청 중 실패한 요청의 비율이다. **왜 필요한가요 · 언제 쓰나요:** 트래픽 증감에 가려지지 않도록 실패를 대상 시도 수와 비교해 신뢰성 변화를 본다.
- **SLI**: 사용자가 받은 결과를 실제 숫자로 재는 방법이다. **왜 필요한가요 · 언제 쓰나요:** 사용자의 성공·실패를 일관되게 측정할 수 있는 운영 신호로 바꿀 때 쓴다.
- **SLO**: 정해진 기간 동안 SLI가 달성해야 할 목표다. **왜 필요한가요 · 언제 쓰나요:** 허용할 서비스 결과에 합의하고 그 목표를 기준으로 작업의 우선순위를 정한다.
- **incident**: 사용자 영향이 발생해 탐지·대응·복구·학습이 필요한 사건이다. **왜 필요한가요 · 언제 쓰나요:** 사용자 피해가 발생한 기간을 중심으로 복구와 사후 학습을 조정할 때 필요하다.
