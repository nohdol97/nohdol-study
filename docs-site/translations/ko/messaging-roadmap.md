...
|---|---|---|
| 메시지(message) | 다른 프로그램에 전달할 데이터 한 건 | 처리 계약이 정해진 작업·이벤트를 프로그램 경계 너머로 전달할 때 쓴다. |
| 생산자(producer) | 메시지를 보내는 프로그램 | 이벤트를 만들고 식별자·스키마·발행 동작을 책임질 주체를 정할 때 필요하다. |
| 소비자(consumer) | 메시지를 받아 처리하는 프로그램 | 전달된 기록을 업무 효과로 바꾸고 재시도·중복을 처리할 주체를 정한다. |
| 큐(queue) | 처리할 메시지를 소비자가 가져갈 때까지 보관하는 줄 | 유입과 처리 속도의 일시적인 차이를 흡수하되 적체 한도 정책을 함께 둔다. |
| 확인 응답(acknowledgement) | 소비자가 처리를 끝냈다고 메시지 시스템에 알리는 신호 | 합의한 완료 지점에 도달했음을 브로커에 알려 전달 진행 상태를 갱신할 때 쓴다. |
| 멱등성(idempotency) | 같은 요청을 여러 번 처리해도 업무 결과가 한 번 처리한 것과 같게 만드는 성질 | 결과를 확신하기 어려운 요청을 재시도해도 업무 효과가 반복되지 않도록 설계할 때 쓴다. |
