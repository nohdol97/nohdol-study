...
|---|---|---|
| 메시지(message) | 다른 프로그램에 전달할 데이터 한 건 | 처리 계약이 정해진 작업·이벤트를 프로그램 경계 너머로 전달할 때 쓴다. **구체적인 상황(가상 예시):** 결제 후 다른 서비스에 영수증 발송을 요청해야 한다. → 고정된 주문 식별자로 메시지를 발행한다. → 수신자가 해석하고 중복을 식별하는지 확인한다. |
| 생산자(producer) | 메시지를 보내는 프로그램 | 이벤트를 만들고 식별자·스키마·발행 동작을 책임질 주체를 정할 때 필요하다. **구체적인 상황(가상 예시):** 주문 이벤트에 소비자가 필요한 필드가 없다. → 생산자의 계약과 호환성 검사를 고친다. → 기존·신규 소비자가 발행 기록을 처리하는지 확인한다. |
| 소비자(consumer) | 메시지를 받아 처리하는 프로그램 | 전달된 기록을 업무 효과로 바꾸고 재시도·중복을 처리할 주체를 정한다. **구체적인 상황(가상 예시):** 메시지는 도착하지만 청구서가 만들어지지 않는다. → 소비자 처리와 출력 커밋을 조사한다. → 수용한 메시지와 영속 청구 기록을 비교한다. |
| 큐(queue) | 처리할 메시지를 소비자가 가져갈 때까지 보관하는 줄 | 유입과 처리 속도의 일시적인 차이를 흡수하되 적체 한도 정책을 함께 둔다. **구체적인 상황(가상 예시):** 할인 행사 동안 작업 유입이 처리보다 잠시 빠르다. → 대기열 용량 정책 안에서 보관한다. → 가장 오래된 항목의 나이와 적체 해소를 추적한다. |
| 확인 응답(acknowledgement) | 소비자가 처리를 끝냈다고 메시지 시스템에 알리는 신호 | 합의한 완료 지점에 도달했음을 브로커에 알려 전달 진행 상태를 갱신할 때 쓴다. **구체적인 상황(가상 예시):** 워커가 결과 쓰기 전에 완료를 알리고 죽는다. → 합의한 영속 완료 지점 뒤에 응답하도록 바꾼다. → 필요한 작업이 유실되지 않는 복구를 시험한다. |
| 멱등성(idempotency) | 같은 요청을 여러 번 처리해도 업무 결과가 한 번 처리한 것과 같게 만드는 성질 | 결과를 확신하기 어려운 요청을 재시도해도 업무 효과가 반복되지 않도록 설계할 때 쓴다. **구체적인 상황(가상 예시):** 서버가 승인한 결제 요청의 응답이 시간 초과된다. → 같은 논리 요청 식별자로 재시도한다. → 결제 효과가 한 번만 기록됐는지 확인한다. |
