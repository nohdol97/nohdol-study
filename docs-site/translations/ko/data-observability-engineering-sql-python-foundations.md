# SQL과 Python: 처리 규모를 늘리기 전에 결과 확인하기

주문 한 건에 상품이 세 개 들어 있다. 주문과 상품을 조인했더니 주문 수가 세 건으로 계산된다. 장비를 늘려도 잘못된 답을 더 빨리 계산할 뿐이다.

먼저 행 하나가 무엇을 나타내는지 정한다. 그다음 작은 SQL·Python 예제로 중복, 누락, 과거 데이터 변경을 확인한다.

## 이 장에서 처음 쓰는 말

| 용어 | 의미 | 왜 필요한가요 · 언제 쓰나요 |
|---|---|---|
| 행 단위(grain) | 행 하나가 나타내는 단위다. 주문 한 건인지, 주문의 상품 한 개를 나타내는 항목인지, 하루 합계인지 구분한다. | 주문 한 건을 여러 번 세거나 서로 다른 단위의 행을 잘못 연결하는 일을 막는다. **구체적인 상황(가상 예시):** 주문 기본 정보와 항목을 조인하자 집계 수가 늘었다. → 의도한 행 단위를 명시·유지한다. → 올바른 행 식별자로 합계를 대조한다. |
| 카디널리티(cardinality) | 값이나 행의 수를 뜻한다. 조인에서는 양쪽 행이 각각 몇 개의 행과 연결될 수 있는지를 본다. | 조인 뒤 행이 예상보다 늘어난 이유를 찾을 때 쓴다. **구체적인 상황(가상 예시):** 일대일이라고 생각한 조인이 추가 행을 만든다. → 양쪽 조인 키의 대응 수를 센다. → 집계를 믿기 전에 중복 키를 찾는다. |
| 윈도 함수(window function) | 고객별 주문 순위처럼 관련 행들을 함께 계산하면서 개별 행은 그대로 남기는 함수다. | 행을 합쳐 없애지 않고 각 행에 순위, 누적합, 앞뒤 값을 붙인다. **구체적인 상황(가상 예시):** 고객별 최신 주문을 다른 열과 함께 조회해야 한다. → 고객 안에서 순서가 명확하도록 주문에 순위를 매긴다. → 시각이 같은 주문까지 포함해 고객마다 한 행이 선택되는지 확인한다. |
| 쿼리 계획(query plan) | 엔진이 데이터를 읽고 쿼리 결과를 만들기 위해 수행할 단계들이다. | 느린 쿼리에서 불필요하게 많이 읽거나 반복하는 작업을 찾는다. **구체적인 상황(가상 예시):** 테이블이 커지자 보고서 조회가 느려졌다. → 실행 계획에서 스캔·조인·예상 행 수를 살핀다. → 원인에 맞게 수정한 뒤 실제 처리량과 시간을 비교한다. |
| 멱등성(idempotence) | 같은 논리적 작업을 반복해도 한 번 수행했을 때와 의도한 결과가 같은 성질이다. | 응답을 못 받아 다시 요청하더라도 결제나 집계가 두 번 반영되지 않게 한다. **구체적인 상황(가상 예시):** 통신 시간 초과로 결제 이벤트가 두 번 전달된다. → 변하지 않는 업무 이벤트 식별자로 중복을 처리한다. → 재전송해도 잔액은 한 번만 바뀌는지 확인한다. |
...
입력에서 c1의 금액을 그대로 더하면 450센트다. 같은 e1이 다시 전달된 것을 한 번만 세면 350센트가 된다. 중복 제거를 빼면 예상값 검사인 단언문이 실패해야 한다. 같은 ID인데 내용이 다른 경우는 단순 중복이 아니므로 종합 프로젝트의 충돌 검사로 따로 처리한다.

## 결과를 이렇게 읽는다

올바른 대상 합계는 350센트다. `c1`을 단순 합산하면 재시도를 세어 450이 된다. NULL 고객은 의도적으로 제외했으므로 모든 입력 주문이 사용자에게 도달했음을 입증하는 쿼리는 아니다. 제외 건수를 별도로 추적한다.

두 번째 `e1`의 금액을 120센트로 바꿔 보자. 같은 이벤트 ID에 서로 다른 금액이 들어왔으므로 위 GROUP BY는 두 행을 하나로 합치지 않는다. 이때는 입력을 거부할지, 버전으로 최신 값을 고를지 업무 규칙을 먼저 정해야 한다. 근거 없이 `MAX(amount_cents)`를 쓰면 큰 금액이 정답이라는 규칙을 임의로 만든 셈이다.
...
이벤트별 최신 상태를 고르려면 어떤 값으로 버전 순서를 정할지 먼저 합의하고 `ROW_NUMBER()`에 사용한다. 시각만 같을 수 있으므로 동률을 구분할 기준도 필요하다. 누적합을 한 행씩 늘리려면 `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`와 순서가 확정되는 정렬 키를 쓴다. 반면 RANGE 프레임은 정렬값이 같은 행들을 함께 계산하므로 합계가 한꺼번에 늘 수 있다. `LAG`는 정렬상 앞 행을 가져온다. 앞 행이 반드시 달력상 어제 기록인 것은 아니다.
...
minute 값이 10인 결제 두 개는 정렬값이 같다. 그래서 RANGE는 첫 행부터 두 금액을 더해 350을 낸다. 재귀 예제에서 `WHERE`를 없애면 종료 조건이 사라진다. 그래프 탐색도 이미 방문한 ID나 최대 깊이를 기록해 같은 경로를 끝없이 돌지 않게 한다. CTE는 쿼리 부분에 붙인 이름이다. 엔진이 그 결과를 임시로 저장해 재사용할지, 참조할 때 다시 계산할지는 실행 방식에 따라 다르므로 비용이 큰 CTE는 계획을 확인한다.
...
쿼리 속도와 별도로, 여러 쿼리가 같은 시점의 데이터를 보는지도 확인한다. PostgreSQL의 Read Committed에서는 같은 트랜잭션 안의 두 문장이 그사이에 커밋된 변경 때문에 다른 데이터를 볼 수 있다. Repeatable Read는 같은 스냅샷을 보지만 여러 행에 걸친 업무 규칙은 별도 설계가 필요할 수 있다. Serializable에서는 충돌한 트랜잭션 전체를 다시 실행해야 할 수 있다. 예를 들어 보고서 생성과 합계 검사가 다른 입력 버전을 읽으면 각 쿼리가 올바르게 실행돼도 숫자가 다를 수 있다. 두 작업이 같은 소스 스냅샷을 사용해야 하는지 정한다.
...
`asyncio`는 한 작업이 네트워크나 DB 응답을 기다리는 동안 다른 작업이 실행되게 한다. 다만 클라이언트가 제어권을 양보해야 한다. 코루틴 안에서도 실행을 막는 동기 클라이언트를 호출하면 이벤트 루프가 멈춘다. 세마포어나 고정된 수의 워커로 동시 요청 수를 제한하고 시간 제한을 둔다. 시간 초과가 나도 원격 쓰기는 이미 끝났을 수 있으므로, 재시도에는 같은 멱등 식별자를 사용한다. `TaskGroup`은 관련 태스크의 시작과 종료를 함께 관리하지만 생성 개수를 제한하지는 않는다. 태스크 100만 개를 만들면 그만큼의 객체도 필요하다.
