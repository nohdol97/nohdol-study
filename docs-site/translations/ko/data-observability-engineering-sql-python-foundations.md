...
| 용어 | 의미 |
|---|---|
| 행 단위(grain) | 주문 항목 하나처럼 행 하나가 나타내는 대상 |
| 카디널리티(cardinality) | 값이나 행의 수. 조인에서는 양쪽이 만들 수 있는 대응 수 |
| 윈도 함수(window function) | 관련 행들을 하나의 집계 행으로 줄이지 않고 함께 계산하는 함수 |
| 쿼리 계획(query plan) | 엔진이 결과를 얻기 위해 수행할 단계 |
| 멱등성(idempotence) | 같은 논리 작업을 반복해도 의도한 결과가 유지되는 성질 |
