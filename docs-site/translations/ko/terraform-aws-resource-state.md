...
- **HCL**: Terraform 설정 파일에 사용하는 문법이다. 보통 `.tf` 파일에 작성한다. **왜 필요한가요 · 언제 쓰나요:** Terraform에서 인프라 블록·입력·참조 관계를 표현할 때 쓴다.
- **resource address**: configuration 안에서 자원 하나를 가리키는 고유한 이름이다. 예: `aws_vpc.main`. **왜 필요한가요 · 언제 쓰나요:** 조회·가져오기·계획 변경의 대상이 되는 정확한 설정 객체를 지정한다.
- **remote object**: AWS처럼 Terraform 밖의 서비스에 실제로 존재하는 자원이다. **왜 필요한가요 · 언제 쓰나요:** 공급자 쪽 실제 객체와 이를 설명하는 로컬 코드·상태를 구분할 때 필요하다.
- **binding**: Terraform 주소와 실제 remote object가 같은 대상이라는 연결 관계다. **왜 필요한가요 · 언제 쓰나요:** 설정 주소가 실수로 다른 기존 객체를 관리하지 않도록 연결 관계를 확인한다.
- **dependency**: 한 자원이 다른 자원의 결과를 필요로 해 먼저 만들어져야 하는 관계다. **왜 필요한가요 · 언제 쓰나요:** 의존하는 작업을 실행하기 전에 필요한 입력이 준비되도록 생성·변경 순서를 정한다.
- **module**: 관련된 Terraform 설정을 입력과 출력이 있는 묶음으로 만든 것이다. **왜 필요한가요 · 언제 쓰나요:** 검토한 인프라 설계를 명시적인 입력과 함께 여러 환경에서 재사용할 때 쓴다.
