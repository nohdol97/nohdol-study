...
- **HCL**: Terraform 설정 파일에 사용하는 문법이다. 보통 `.tf` 파일에 작성한다. **왜 필요한가요 · 언제 쓰나요:** Terraform에서 인프라 블록·입력·참조 관계를 표현할 때 쓴다. **구체적인 상황(가상 예시):** 환경별 값이 설정 여러 곳에 반복된다. → HCL 입력과 참조로 표현한다. → 유효성 검사와 계획을 확인한다.
- **resource address**: configuration 안에서 자원 하나를 가리키는 고유한 이름이다. 예: `aws_vpc.main`. **왜 필요한가요 · 언제 쓰나요:** 조회·가져오기·계획 변경의 대상이 되는 정확한 설정 객체를 지정한다. **구체적인 상황(가상 예시):** 가져오기가 잘못된 Terraform 객체를 대상으로 한다. → 전체 자원 주소와 설정을 맞춘다. → 추가 변경 전에 연결 결과를 확인한다.
- **remote object**: AWS처럼 Terraform 밖의 서비스에 실제로 존재하는 자원이다. **왜 필요한가요 · 언제 쓰나요:** 공급자 쪽 실제 객체와 이를 설명하는 로컬 코드·상태를 구분할 때 필요하다. **구체적인 상황(가상 예시):** 코드에 버킷을 선언했지만 운영자가 서비스 콘솔에서 설정을 바꿨다. → 공급자 쪽 객체 식별자와 현재 설정을 확인한다. → 설정·state·실제 객체의 차이를 설명한다.
- **binding**: Terraform 주소와 실제 remote object가 같은 대상이라는 연결 관계다. **왜 필요한가요 · 언제 쓰나요:** 설정 주소가 실수로 다른 기존 객체를 관리하지 않도록 연결 관계를 확인한다. **구체적인 상황(가상 예시):** 설정 주소 하나가 다른 서버에 연결된 듯하다. → 기록된 원격 ID와 목록을 비교한다. → 갱신 전에 연결을 바로잡는다.
- **dependency**: 한 자원이 다른 자원의 결과를 필요로 해 먼저 만들어져야 하는 관계다. **왜 필요한가요 · 언제 쓰나요:** 의존하는 작업을 실행하기 전에 필요한 입력이 준비되도록 생성·변경 순서를 정한다. **구체적인 상황(가상 예시):** 애플리케이션 자원에 같은 계획이 만드는 네트워크가 필요하다. → 실제 참조 의존성을 선언한다. → 계획이 해당 의존성을 따르는지 확인한다.
- **module**: 관련된 Terraform 설정을 입력과 출력이 있는 묶음으로 만든 것이다. **왜 필요한가요 · 언제 쓰나요:** 검토한 인프라 설계를 명시적인 입력과 함께 여러 환경에서 재사용할 때 쓴다. **구체적인 상황(가상 예시):** 환경 세 개가 거의 같은 네트워크 설정을 복사한다. → 명시적 입력을 받는 모듈로 묶는다. → 환경별 계획에 의도하지 않은 차이가 없는지 확인한다.
