...
두 번째 Pod는 마커를 다시 쓰지 않고 `written-before-pod-replacement`를 읽어야 한다. 교체 전후 Pod UID는 다르고 PVC UID는 같은지 기록한다. 매번 시작할 때 같은 파일을 쓰면 영속성이 깨져도 실험이 통과할 수 있다. 이 검사는 해당 Pod/PVC 수명 주기만 입증하며 백업 복구는 입증하지 않는다. 완료 후 PV 회수 정책을 확인하고 `kubectl delete -f storage.yaml`로 임시 실습 리소스만 삭제한다. Retain 볼륨은 별도 정리가 필요하다.
...
삭제 전후 Pod UID를 기록한다. Pod UID는 달라지고 PVC 식별자는 같아야 한다. 새로 실행된 프로세스는 마커를 쓰지 않으므로 동일한 내용이 남아 있으면 해당 Pod 교체를 넘어 데이터가 유지됐음을 보여 준다. PVC가 Pending이거나 파일이 없으면 실습 실패다. 이것만으로 백업 복구나 모든 스토리지 드라이버의 노드 간 접근이 입증되지는 않는다.
...
[← Service와 네트워킹](../../content/kubernetes/05-services-and-networking.md) · [스케줄링과 리소스·오토스케일링 →](../../content/kubernetes/07-scheduling-and-autoscaling.md)
