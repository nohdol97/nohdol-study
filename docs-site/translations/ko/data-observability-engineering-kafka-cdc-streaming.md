...
|---|---|---|
| 파티션 / 오프셋 | 순서가 있는 로그 구간 / 그 안의 위치 | 순서 보장의 범위를 정하고 중단 후 각 소비자가 재개할 위치를 기록한다. |
| 소비자 그룹(consumer group) | 파티션 소비 책임을 나눠 갖는 소비자들 | 여러 워커가 파티션 처리를 나누고 참여자가 바뀌면 담당을 재배치한다. |
| 재균형(rebalance) | 소비자 사이 파티션 할당의 변경 | 소비자의 참여·이탈 때 파티션을 재분배하면서 중단과 중복 처리를 확인한다. |
| CDC | Change Data Capture, 변경 데이터 캡처. 데이터베이스 변경을 하위 시스템이 사용할 레코드로 바꾸는 것 | 원본 모든 행을 반복 복사하는 대신 데이터베이스 변경으로 하위 복사본을 갱신한다. |
| 이벤트 시간 / 처리 시간 | 이벤트가 발생한 시각 / 처리기가 다룬 시각 | 시간 기준 결과를 만들 때 업무 사건의 발생 시점과 도착 지연을 구분한다. |
| 워터마크(watermark) | 지원되는 상태 유지 연산에 사용하는 이벤트 시간 진척 경계 | 명시한 정책에 따라 지원되는 스트리밍 연산의 상태 보존과 늦은 데이터 처리를 제한한다. |
| 체크포인트(checkpoint) | 처리기 복구에 필요한 진척과 상태 기록 | 장애 후 보존한 진행 상태·연산 상태에서 재개하되 출력의 정확성은 별도로 확인한다. |
