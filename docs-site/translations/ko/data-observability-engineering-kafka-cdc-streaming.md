...
|---|---|---|
| 파티션 / 오프셋 | 순서가 있는 로그 구간 / 그 안의 위치 | 순서 보장의 범위를 정하고 중단 후 각 소비자가 재개할 위치를 기록한다. **구체적인 상황(가상 예시):** Kafka 소비자가 장애 후 이어서 처리해야 한다. → 파티션별 읽기 위치를 기록하고 복구한다. → 오프셋 커밋 경계에 맞게 재처리와 중복 처리가 되는지 확인한다. |
| 소비자 그룹(consumer group) | 파티션 소비 책임을 나눠 갖는 소비자들 | 여러 워커가 파티션 처리를 나누고 참여자가 바뀌면 담당을 재배치한다. **구체적인 상황(가상 예시):** 소비자 하나가 토픽 처리량을 따라가지 못한다. → 파티션 병렬성 한도 안에서 같은 그룹에 소비자를 추가한다. → 선형 확장을 가정하지 말고 할당과 지연을 확인한다. |
| 재균형(rebalance) | 소비자 사이 파티션 할당의 변경 | 소비자의 참여·이탈 때 파티션을 재분배하면서 중단과 중복 처리를 확인한다. **구체적인 상황(가상 예시):** Kafka 소비자를 추가하자 파티션 담당 프로세스가 바뀐다. → 그룹 재균형을 관찰하고 할당 회수를 안전하게 처리한다. → 처리되지 않은 중복이나 진행 위치 유실 없이 재개되는지 확인한다. |
| CDC | Change Data Capture, 변경 데이터 캡처. 데이터베이스 변경을 하위 시스템이 사용할 레코드로 바꾸는 것 | 원본 모든 행을 반복 복사하는 대신 데이터베이스 변경으로 하위 복사본을 갱신한다. **구체적인 상황(가상 예시):** 주문이 결제 대기에서 완료로 바뀌었는데 분석 화면에는 대기로 남아 있다. → DB 변경을 포착해 주문 식별자로 분석 저장소에 반영한다. → 반영 지연을 확인하고 재전송해도 주문이 늘지 않는지 확인한다. |
| 이벤트 시간 / 처리 시간 | 이벤트가 발생한 시각 / 처리기가 다룬 시각 | 시간 기준 결과를 만들 때 업무 사건의 발생 시점과 도착 지연을 구분한다. **구체적인 상황(가상 예시):** 휴대전화가 오프라인에서 발생한 이벤트를 한참 뒤 업로드한다. → 발생 시각과 도착·처리 시각을 따로 보존한다. → 보고서가 의도한 업무 시각 기준으로 집계하는지 확인한다. |
| 워터마크(watermark) | 지원되는 상태 유지 연산에 사용하는 이벤트 시간 진척 경계 | 명시한 정책에 따라 지원되는 스트리밍 연산의 상태 보존과 늦은 데이터 처리를 제한한다. **구체적인 상황(가상 예시):** 지난 스트리밍 구간에 해당하는 이벤트가 계속 늦게 도착한다. → 엔진이 지원하는 워터마크와 지연 데이터 정책을 정한다. → 허용 지연 경계의 양쪽 이벤트를 시험한다. |
| 체크포인트(checkpoint) | 처리기 복구에 필요한 진척과 상태 기록 | 장애 후 보존한 진행 상태·연산 상태에서 재개하되 출력의 정확성은 별도로 확인한다. **구체적인 상황(가상 예시):** 상태를 가진 스트리밍 잡이 실패 후 재시작한다. → 설정한 체크포인트에서 진행 위치와 상태를 복구한다. → 소스 재생과 싱크 동작이 의도한 처리 계약을 지키는지 확인한다. |
