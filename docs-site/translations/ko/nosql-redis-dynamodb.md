...
- **source of truth**: 다른 복사본이 틀렸을 때 최종적으로 맞다고 판단할 원본 데이터다. **왜 필요한가요 · 언제 쓰나요:** 캐시·복제본 등 여러 사본이 다를 때 기준 기록으로 판단을 내리기 위해 쓴다.
- **cache**: 느린 원본 조회를 줄이기 위해 결과를 잠시 가까이 보관한 복사본이다. **왜 필요한가요 · 언제 쓰나요:** 정해진 기간 안전하게 재사용할 결과를 보관해 원본의 반복 작업을 줄인다.
- **eviction**: memory가 부족할 때 정해진 정책으로 일부 key를 제거하는 동작이다. **왜 필요한가요 · 언제 쓰나요:** 버려도 되는 항목을 선택해 메모리가 제한된 캐시의 운영을 유지한다.
- **persistence**: process가 종료되거나 재시작해도 데이터를 복구할 수 있도록 별도 저장 장치에 남기는 방식이다. **왜 필요한가요 · 언제 쓰나요:** 프로세스 메모리에만 의존하지 않고 재시작 후 필요한 상태를 복구할 때 쓴다.
- **partition**: 많은 데이터를 여러 저장 구역에 나누어 두는 단위다. **왜 필요한가요 · 언제 쓰나요:** 저장·처리 작업을 나누되 그에 따른 순서·접근 경계를 명시할 때 쓴다.
- **secondary index**: 기본 key가 아닌 다른 조건으로 데이터를 찾기 위해 유지하는 추가 색인이다. **왜 필요한가요 · 언제 쓰나요:** 기본 키 배치만으로는 전체 스캔이 필요한 추가 조회 방식을 지원할 때 쓴다.
