...
- **eviction**: memory가 부족할 때 정해진 정책으로 일부 key를 제거하는 동작이다. **왜 필요한가요 · 언제 쓰나요:** 버려도 되는 항목을 선택해 메모리가 제한된 캐시의 운영을 유지한다. **구체적인 상황(가상 예시):** 새 키가 들어오는데 캐시 메모리가 가득 찬다. → 퇴출 정책을 검토·시험한다. → 캐시 실패율과 필수 데이터의 별도 보관 여부를 확인한다.
- **persistence**: process가 종료되거나 재시작해도 데이터를 복구할 수 있도록 별도 저장 장치에 남기는 방식이다. **왜 필요한가요 · 언제 쓰나요:** 프로세스 메모리에만 의존하지 않고 재시작 후 필요한 상태를 복구할 때 쓴다. **구체적인 상황(가상 예시):** 캐시를 사용하는 서비스가 예기치 않게 재시작된다. → 설정한 영속성 계약에 따라 복구한다. → 승인된 갱신의 보존 범위와 허용 유실을 확인한다.
