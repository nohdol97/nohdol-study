...
중첩된 revision을 지우거나 시간 구간을 뒤집어도 첫 줄은 여전히 `missing= []`다. 의도한 반례이며 원래 명령은 키 존재만 검사했다는 뜻이다. 의미 검토는 진단 전에 이렇게 바뀐 기록을 거부해야 한다.
...
- 이 bundle을 [이상 탐지와 진단](../../content/aiops-diagnosis/01-detection-correlation-rca.md)에 넘기기 전에 어떤 품질 지표를 집계할 것인가?
