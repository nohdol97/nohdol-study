...
|---|---|---|
| 구성(configuration) | 만들고 싶은 자원을 코드로 적은 파일 | 기억에 의존한 수동 절차 대신 인프라 의도를 검토·버전 관리·재현할 때 쓴다. |
| 자원(resource) | Terraform이 하나의 단위로 생성·조회·변경하는 대상 | Terraform이 생성·변경을 계획할 수 있도록 관리할 인프라 단위를 선언한다. |
| 공급자(provider) | Terraform 요청을 AWS 같은 외부 서비스 API로 전달하는 플러그인 | 선언한 자원을 대상 플랫폼의 API 작업으로 연결할 때 쓴다. |
| 상태(state) | 코드의 자원과 실제 AWS 자원이 서로 같은 대상임을 기억하는 기록 | 설정 주소와 기존 객체를 연결해 변경이 의도한 자원에 적용되도록 한다. |
| 계획(plan) | 지금 적용하면 무엇이 생성·변경·삭제될지 보여 주는 제안서 | 상태를 바꾸는 실행을 허용하기 전에 대상·차이·가정을 검토할 때 쓴다. |
| 적용(apply) | 검토한 계획을 실제 외부 서비스에 요청하는 단계 | 계정과 범위를 확인한 인프라 변경 계획을 실제로 실행할 때 쓴다. |
