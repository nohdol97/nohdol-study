...
|---|---|---|
| 구성(configuration) | 만들고 싶은 자원을 코드로 적은 파일 | 기억에 의존한 수동 절차 대신 인프라 의도를 검토·버전 관리·재현할 때 쓴다. **구체적인 상황(가상 예시):** 수동 설정한 서버를 일관되게 재생성하지 못한다. → 의도한 인프라 설정을 기록한다. → 새 계획을 검토한 설계와 비교한다. |
| 자원(resource) | Terraform이 하나의 단위로 생성·조회·변경하는 대상 | Terraform이 생성·변경을 계획할 수 있도록 관리할 인프라 단위를 선언한다. **구체적인 상황(가상 예시):** 환경에 관리 대상 저장 버킷이 필요하다. → Terraform 리소스로 선언하고 실행 계획을 검토한다. → 생성된 객체와 상태가 의도한 설정에 맞는지 확인한다. |
| 공급자(provider) | Terraform 요청을 AWS 같은 외부 서비스 API로 전달하는 플러그인 | 선언한 자원을 대상 플랫폼의 API 작업으로 연결할 때 쓴다. **구체적인 상황(가상 예시):** Terraform이 잘못된 API 엔드포인트로 접근한다. → 선택된 provider 설정을 조사한다. → 적용 전에 계정·리전·계획 대상을 확인한다. |
| 상태(state) | 코드의 자원과 실제 AWS 자원이 서로 같은 대상임을 기억하는 기록 | 설정 주소와 기존 객체를 연결해 변경이 의도한 자원에 적용되도록 한다. **구체적인 상황(가상 예시):** 이미 있는 객체가 다시 생성 대상으로 나온다. → 설정과 state의 대응을 조사한다. → 중복 생성을 승인하기 전에 연결을 바로잡는다. |
| 계획(plan) | 지금 적용하면 무엇이 생성·변경·삭제될지 보여 주는 제안서 | 상태를 바꾸는 실행을 허용하기 전에 대상·차이·가정을 검토할 때 쓴다. **구체적인 상황(가상 예시):** 용량 추가 변경이 삭제까지 제안한다. → 계획의 정확한 대상과 차이를 검토한다. → 승인 전에 예상하지 못한 삭제를 해결한다. |
| 적용(apply) | 검토한 계획을 실제 외부 서비스에 요청하는 단계 | 계정과 범위를 확인한 인프라 변경 계획을 실제로 실행할 때 쓴다. **구체적인 상황(가상 예시):** 격리 시험 환경용 계획의 검토가 끝났다. → 의도한 신원으로 적용한다. → 실제 자원과 후속 계획을 기대 상태와 비교한다. |
...
1. [Resource graph와 state](../../content/terraform-aws/01-resource-graph-and-state.md): resource address, dependency, provider, state와 remote backend의 책임을 구분한다.
2. [Plan, drift와 import 실습](../../content/terraform-aws/02-plan-drift-import-lab.md): cloud 변경 없이 core workflow를 실행하고 AWS plan의 승인·복구 절차를 설계한다.
...
- [Helm Charts와 GitOps](../../content/helm-gitops/00-roadmap.md)와 같은 resource를 동시에 관리하지 않도록 ownership을 정한다.
