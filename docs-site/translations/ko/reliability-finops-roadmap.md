...
|---|---|---|
| 가용성(availability) | 사용자가 필요한 기능을 실제로 사용할 수 있었던 정도 | 서버 실행 여부를 넘어 사용자가 필요한 행동을 완료할 수 있는지 측정할 때 쓴다. |
| 장애(failure) | 시스템의 일부가 기대한 기능을 수행하지 못하는 상태 | 고장 난 기능을 구체화해 복구가 사용자에게 보이는 문제를 해결하도록 한다. |
| 중복 구성(redundancy) | 하나가 실패해도 계속 동작하도록 같은 역할을 여러 곳에 두는 설계 | 구성 요소 하나가 고장 나도 대체 제공 경로를 남기며 공유 의존성도 확인한다. |
| RTO | 장애가 시작된 뒤 서비스를 다시 사용할 수 있을 때까지 허용하는 목표 시간 | 허용할 최대 중단 시간에 맞는 복구 방식을 선택하고 연습할 때 쓴다. |
| RPO | 복구 과정에서 과거로 돌아가 잃을 수 있다고 허용한 데이터 시간 범위 | 얼마나 최근의 진행 상태를 잃어도 되는지에 따라 백업·복제 요구를 정한다. |
| 용량(capacity) | 시스템이 감당할 수 있는 요청·저장·처리의 양 | 예상 수요와 장애 상황을 현재 처리·저장 용량으로 감당할 수 있는지 확인한다. |
