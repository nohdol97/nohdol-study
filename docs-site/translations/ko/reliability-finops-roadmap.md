...
|---|---|---|
| 가용성(availability) | 사용자가 필요한 기능을 실제로 사용할 수 있었던 정도 | 서버 실행 여부를 넘어 사용자가 필요한 행동을 완료할 수 있는지 측정할 때 쓴다. **구체적인 상황(가상 예시):** 서버 상태 검사는 통과하지만 사용자는 주문할 수 없다. → 성공한 주문 시도를 측정한다. → 사용자 가용성과 인프라 상태를 비교한다. |
| 장애(failure) | 시스템의 일부가 기대한 기능을 수행하지 못하는 상태 | 고장 난 기능을 구체화해 복구가 사용자에게 보이는 문제를 해결하도록 한다. **구체적인 상황(가상 예시):** 결제는 성공하지만 영수증 전달만 실패한다. → 고장 난 기능과 의존성을 찾는다. → 결제를 방해하지 않고 영수증 기능의 복구를 확인한다. |
| 중복 구성(redundancy) | 하나가 실패해도 계속 동작하도록 같은 역할을 여러 곳에 두는 설계 | 구성 요소 하나가 고장 나도 대체 제공 경로를 남기며 공유 의존성도 확인한다. **구체적인 상황(가상 예시):** 복제본 두 개가 같은 고장 난 저장소에 의존한다. → 실제 장애 경계별 중복성을 검토한다. → 해당 의존성 손실을 대체 경로가 견디는지 시험한다. |
| RTO | 장애가 시작된 뒤 서비스를 다시 사용할 수 있을 때까지 허용하는 목표 시간 | 허용할 최대 중단 시간에 맞는 복구 방식을 선택하고 연습할 때 쓴다. **구체적인 상황(가상 예시):** 서비스가 30분 안에 복구돼야 한다. → 탐지·복원·검증을 하나의 시간표로 연습한다. → 측정한 전체 시간을 RTO와 비교한다. |
| RPO | 복구 과정에서 과거로 돌아가 잃을 수 있다고 허용한 데이터 시간 범위 | 얼마나 최근의 진행 상태를 잃어도 되는지에 따라 백업·복제 요구를 정한다. **구체적인 상황(가상 예시):** 복구 요구가 진행 상태 5분의 유실을 허용한다. → 백업·복제의 복구 지점을 조사한다. → 실제 복구 가능한 지점이 한도를 만족하는지 확인한다. |
| 용량(capacity) | 시스템이 감당할 수 있는 요청·저장·처리의 양 | 예상 수요와 장애 상황을 현재 처리·저장 용량으로 감당할 수 있는지 확인한다. **구체적인 상황(가상 예시):** 캠페인으로 트래픽이 두 배가 될 수 있다. → 대표 부하와 하위 시스템 한도를 시험한다. → 지연 요구를 만족하는 지속 처리량을 기록한다. |
