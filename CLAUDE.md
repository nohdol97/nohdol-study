...
- Route selected sources through `ingest`; keep `/watch` frame analysis explicit-use only.
- The `understand` skill routes to its nine modes internally; its graphs are navigation. Confirm any factual answer in the source file before finishing, and keep the dashboard and Figma paths explicit-use only.
