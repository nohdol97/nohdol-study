...
`REGISTRY.md` is the installation-specific source of truth for the knowledge-root path, profile, sync method, and observed local capabilities. It is intentionally untracked.
...
Write one source line per paragraph, list item, and blockquote line. Do not hard-wrap prose at a column: the break renders as a space, so it changes nothing a reader sees while making every later edit reflow the block and every diff show rewrapping instead of the change. A line break belongs to structure only - a heading, list item, table row, fenced code, frontmatter key, or an explicit hard break.
...
- An MCP server that executes code or sends data on an external service (for example the Colab MCP server) is optional third-party transmission. Never install or register one on a `corporate` installation; on `personal`, `study-install` offers it as an explicit opt-in check and records the decision in `REGISTRY.md`. Vault content never goes through such a server without separate explicit approval.
- The permission prompt does not enforce the previous rule: it asks whether to run a tool, not whether the payload carries the user's notes. `.agents/hooks/study-egress-guard.py` is that enforcement for the notebook-cell tools, it runs on every surface rather than only prompt-less ones, and it denies a cell carrying a knowledge-root path, a vault-relative path, a wikilink, or a note's frontmatter. Figures measured from a public dataset are not vault material and stay allowed. Treat the guard as a targeted check on one known channel, never as proof that a transfer is safe: paraphrased or encoded content passes it, so the judgment stays with the agent.
...
- A surface that runs the CLI with permission prompts disabled has no gate on what a tool call touches, so it reads the knowledge root without writing to it and never sweeps the home directory. Whether a note is worth keeping, whether it duplicates one that exists, and whether the index, log, and cache moved with it are judgment calls that need a person at the prompt, so knowledge changes belong to an interactive session while the prompt-less surface searches, reads, explains, and answers. Register `.agents/hooks/study-tool-guard.py` through `study-install` before driving the harness from such a surface, and have the surface mark itself with `STUDY_SURFACE`.
...
- This repository has standing user authorization to commit and push completed, freshly verified changes to `origin/main` without a separate confirmation. Inspect the status and diff before delivery. This authorization does not cover force pushes, history rewrites, destructive Git operations, releases, secrets, or a different remote or branch; those still require explicit scope.
...
Phase 1 is the portable filesystem harness, installation workflow, note contract, and session context. Phase 2 provides web, paper, and video ingest plus a deterministic Markdown graph baseline. Phase 2b adds project-local Understand Anything and Obsidian skills: the verified source-pin installer, the `understand` skill routing all nine upstream entry points, the typed knowledge graph, and the `obsidian` format and CLI skill are in place. A generated graph is navigation, never evidence: confirm a factual answer in the source file before finishing. Running the adapters that need a built dependency set stays blocked until that install is separately authorized. Phase 2c may evaluate basic-memory only in an explicitly scoped, read/search-focused corpus with original-file hash checks; there is no arbitrary note-count gate. Phase 3 adds diagrams, guided study sessions, gardening, and recall. A diagram artifact that a note cannot embed - the standalone interactive HTML the pinned `archify` CLI renders - is explicit-use only, is written under `_workspace/` and never into the knowledge root, and does not change how a note-bound diagram is drawn. Two rules they share: a tool that walks the knowledge root scans the curated layer only, since a real vault holds unrelated directories and on cloud storage a full walk is slow and pointless - reading the filenames under `raw/` is the single exception, because Obsidian resolves a wikilink against the whole vault and a link naming a capture would otherwise be reported broken, so names alone are read and nothing there becomes a node; and a derived artifact - a diagram, a report, a review card - explains or reports, and never becomes evidence on its own. Do not silently pull later-phase dependencies across these boundaries.
